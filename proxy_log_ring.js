'use strict';
/**
 * proxy_log_ring.js — in-memory ring buffer of the proxy's own console output (+ a pm2 stats reader).
 * Ported verbatim from the email_queue admin (src/.../admin/log_ring.js) so the proxy /admin Logs panel
 * behaves identically.
 *
 * install(console) patches console.log/info/warn/error so every line is ALSO captured into a ~500-line
 * ring (the real console still prints normally). The /admin "Server console" reads the ring via SSE, so
 * you can watch live server feedback in the browser. Works in dev (node server_proxy_8000.js) AND under
 * pm2 — it does NOT depend on pm2 log files existing, which is why it populates when the file tail can't.
 */
