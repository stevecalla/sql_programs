...
// Patterned on src/salesforce_email_queue_proof_of_concept/menu.js: numbered
// color-coded sections, a .menu_prefs.json toggle to show the underlying CLI, a
// titled banner before each run, and a spawn-based run harness where Ctrl-C stops
// the child and returns to the menu. Built on Node's readline — no dependencies.
//
// Health checks call the running proxy directly (global fetch), so start the
// proxy first (Proxy server section). pm2 actions shell out to npm scripts.
//
// Usage:  node menu.js          (interactive)
//         node menu.js test     (run proxy tests headless, e.g. for CI)
...
// Spawn a child command; Ctrl-C goes to the child, menu resumes when it closes.
function run(cmd, args) {
  return new Promise(function (resolve) {
    ...
    // down cleanly. Only npm/npx and similar need a shell on Windows.
...
// Fetch JSON from the running proxy and pretty-print it.
async function get_json(pathname) {
  ...
  try {
    ...
    console.log(c(color, '  HTTP ' + r.status));
...
// Run a command and capture its stdout (used to parse `pm2 jlist`).
...
// List pm2 process names, let the user pick one by number, and restart it.
async function pick_and_restart() {
  ...
  try {
    ...
    const arr = JSON.parse(a >= 0 ? out.slice(a, b + 1) : out);
    names = arr.map(function (x) { return x.name; });
  } catch (e) {
    console.log(c(RED, '  Could not read the pm2 list (is pm2 running?): ' + e.message));
    return;
  }
...
function print_reminders() {
  const tips = [
    ...
    [GRAY, '  1) edit proxy_routes.js (uncomment or add a { target, health } line)'],
    [GRAY, '  2) npm run pm2_reload_proxy        (zero-downtime; re-reads the route map)'],
    [GRAY, '  3) test usat-api.kidderwise.org/<prefix>/<endpoint>'],
    [YEL, 'Restart vs reload the proxy'],
    [GRAY, '  reload  = npm run pm2_reload_proxy  (recycles workers one at a time, no blip)'],
    [GRAY, '  restart = npm run restart_proxy     (hard; ~1s blip — use if a worker is stuck)'],
    [GRAY, '  You only recycle the proxy for route/proxy-code changes. Deploying a backend'],
    [GRAY, '  app (pm2 restart <name>) needs NO proxy restart.'],
    [YEL, 'Verify everything is up'],
    [GRAY, '  Health & status menu -> "All backends health" (or GET /api/health).'],
    [GRAY, '  Health passing through the proxy => that backend\'s old subdomain can retire.'],
    [YEL, 'Repoint a Slack command to the proxy'],
    [GRAY, '  In api.slack.com -> [app] -> Slash Commands, change the Request URL to'],
    [GRAY, '  usat-api.kidderwise.org/<prefix>/<same-path>. Command name + backend stay the same.'],
    [YEL, 'Cron jobs'],
    [GRAY, '  Unaffected — they call http://localhost:<port> directly, bypassing the proxy.'],
    [YEL, 'Open the VS Code log terminals (multi-pane)'],
    [GRAY, '  Ctrl+Shift+P -> "Tasks: Run Task" -> "All Logs (18 groups)"  (or "Test").'],
    [GRAY, '  Tip: Ctrl+P then type "task " also lists them. Opens one tail terminal per server.'],
    [YEL, 'Open the admin console'],
    [GRAY, '  Start the proxy, then browse ' + PROXY_BASE + '/admin (or use the Health menu item).'],
    [GRAY, '  Login uses PROXY_ADMIN_USER / PROXY_ADMIN_PASS from .env.'],
    [YEL, 'Install the optional rate limiter (on the server)'],
    [GRAY, '  npm i express-rate-limit   (proxy runs fine without it; then reload)']
  ];
  console.log('');
  tips.forEach(function (t) { console.log('  ' + c(t[0], t[1])); });
  console.log('');
}

const SECTIONS = [
  { label: 'Tests', color: MAGENTA, items: [
    { label: 'Run proxy tests', desc: 'node --test on the proxy suite (endpoints, 404, 405).', cli: 'npm run test_proxy', action: 'test_proxy' }
  ] },
  { label: 'Start the local server', color: GREEN, items: [
    { label: 'Run proxy in FOREGROUND (dev)', desc: 'node server_proxy_8000.js — Ctrl-C to stop. Best for local testing.', cli: 'node server_proxy_8000.js', action: 'p_fg' },
    { label: 'Start proxy under pm2 (cluster, 2 workers)', desc: 'Background, production-style, survives crashes.', cli: 'npm run pm2_start_proxy', action: 'p_start' }
  ] },
  { label: 'Health & status', color: CYAN, items: [
    { label: 'Proxy alive (/api/test)', desc: 'No-backend smoke check that the proxy is up.', cli: 'curl ' + PROXY_BASE + '/api/test', action: 'h_test' },
    { label: 'Proxy status (/api/status)', desc: 'Uptime, memory, node, enabled routes.', cli: 'curl ' + PROXY_BASE + '/api/status', action: 'h_status' },
    { label: 'All backends health (/api/health)', desc: 'Pings every enabled backend; one up/down report.', cli: 'curl ' + PROXY_BASE + '/api/health', action: 'h_health' },
    { label: 'Open admin console (browser)', desc: 'Opens ' + PROXY_BASE + '/admin (start the proxy first; login required).', cli: 'open ' + PROXY_BASE + '/admin', action: 'open_admin' }
  ] },
  { label: 'Proxy control (pm2)', color: BLU, items: [
    { label: 'Reload proxy (zero-downtime)', desc: 'Use after editing proxy_routes.js or the proxy code.', cli: 'npm run pm2_reload_proxy', action: 'p_reload' },
    { label: 'Restart proxy (hard)', desc: 'Full restart; brief blip. Use if a worker is stuck.', cli: 'npm run restart_proxy', action: 'p_restart' },
    { label: 'Stop proxy', desc: 'pm2 stop usat_proxy.', cli: 'npm run stop_proxy', action: 'p_stop' },
    { label: 'Show proxy', desc: 'pm2 show usat_proxy (status, restarts, memory).', cli: 'npm run show_proxy', action: 'p_show' }
  ] },
  { label: 'Fleet (all servers)', color: RED, items: [
    { label: 'Start ALL servers (pm2)', desc: 'npm run pm2_run_all_servers — start the whole fleet in order.', cli: 'npm run pm2_run_all_servers', action: 'fleet_start_all' },
  ...
  { label: 'Logs (no SSH needed)', color: MAGENTA, items: [
    { label: 'List pm2 processes', desc: 'All running servers + status/restarts/memory.', cli: 'npx pm2 list', action: 'l_list' },
    { label: 'Tail proxy logs', desc: 'npx pm2 logs usat_proxy.', cli: 'npm run logs_proxy', action: 'l_proxy' },
    { label: 'Tail ALL logs', desc: 'npx pm2 logs (every process, interleaved).', cli: 'npx pm2 logs', action: 'l_all' },
    { label: 'Tail one server by name', desc: 'Prompts for the pm2 name (e.g. usat_slack).', cli: 'npx pm2 logs <name>', action: 'l_pick' }
  ] },
  { label: 'Help', color: YEL, items: [
    { label: 'Reminders / cheat-sheet', desc: 'Add a route, reload vs restart, repoint Slack, verify health.', cli: '', action: 'reminders' }
  ] }
];
...
function render() {
  ...
  SECTIONS.forEach(function (sec) {
    ...
    sec.items.forEach(function (it) {
      n += 1; map[n] = it;
      // One line per item: [n] Label — description (CLI on a 2nd line only when shown).
