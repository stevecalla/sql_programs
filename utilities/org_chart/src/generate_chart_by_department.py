# org_chart_by_dept_responsive_with_cover_ordered_pagenums_wrapped_tenure.py
import math
import pandas as pd
...
OUTPUT = "org_chart_usat_by_dept_responsive_ordered_wrapped_tenure.pptx"

# ---------- Config ----------
ORDER_COL = "order"  # change if your ordering column is named differently

# ---------- Load & validate ----------
df = pd.read_csv(INPUT, dtype=str).fillna("")
need = ["employee_id", "name", "title", "manager_id", "department", "tenure"]
for c in need:
...
# Optionally coerce order column to numeric for ordering, if it exists
if ORDER_COL in df.columns:
    order_map = (
        df.groupby("department")[ORDER_COL]
          .first()
...
TOP_M_IN, BOTTOM_M_IN = 0.7, 0.4       # slightly tighter margins
TITLE_H_IN = 0.5

# Target (unscaled) box size & gaps
TARGET_BOX_W_IN = 2.4
TARGET_BOX_H_IN = 1.0
TARGET_X_GAP_IN = 2.6
TARGET_Y_GAP_IN = 1.4

# Minimums to keep readability (compact mode when needed)
MIN_BOX_W_IN = 1.2
MIN_BOX_H_IN = 0.75
MIN_X_GAP_IN = 0.35
MIN_Y_GAP_IN = 0.4

# Font sizes (will scale down slightly if we’re tight)
NAME_FONT_PT = 12
TITLE_FONT_PT = 10
TENURE_FONT_PT = 9
NAME_FONT_MIN_PT = 9
...
def draw_person_box(slide, x, y, w, h, name, title, tenure, name_pt, title_pt, tenure_pt,
    ...
    box = slide.shapes.add_shape(1, x, y, w, h)  # rectangle
    box.fill.solid()
...
def draw_line(slide, x, y, w, h, color=(100,100,100)):
    if w < 0: x = x + w; w = -w
    if h < 0: y = y + h; h = -h
    seg = slide.shapes.add_shape(1, x, y, max(Emu(1), w), max(Emu(1), h))
...
def draw_department_slide(prs, dept_name, sub_df, page_num, total_pages):
    """Department slide with wrapping for wide levels and tenure display and de-overlapped connectors."""
    ...
    title_shape = slide.shapes.add_shape(1, LEFT_M, Inches(0.2), prs.slide_width - LEFT_M - RIGHT_M, TITLE_H)
        ...
        for col, node_id in enumerate(nodes):
            if is_ext:
                ...
                draw_person_box(slide, x, current_top_y, box_w, box_h,
                                mgr_row.name, mgr_row.title, getattr(mgr_row, "tenure", ""),
    ...
    def safe_bus_y(m_bottom_y, s_top_y):
        """Clamp bus between boxes with small padding."""
    ...
    def bus_y_for(parent_id, m_bottom_y, s_top_y):
        ...
        # Deterministic small jitter per parent (row+col+id hash) to avoid shared buses
        seed = (row_index_by_id.get(parent_id, 0) * 131 + col_index_by_id.get(parent_id, 0) * 17 + hash(parent_id)) & 0xFFFFFFFF
        lane = (seed % JITTER_BUCKETS) - (JITTER_BUCKETS // 2)  # e.g., -2..+2 when 5 buckets
        y = base + lane * JITTER_STEP
        # Clamp again inside the gap
        top = m_bottom_y + GAP_SAFE
        bottom = s_top_y - GAP_SAFE
...
def draw_cover_slide(prs, dept_ordered, dept_counts, total_employees, page_num, total_pages, df):
    ...
    # Title
    title_shape = slide.shapes.add_shape(1, LEFT_M, Inches(0.2),
...
def get_department_order(df):
    ...
    def key_func(d):
        if d in order_map and pd.notnull(order_map[d]):
            return (0, float(order_map[d]), d.lower())
