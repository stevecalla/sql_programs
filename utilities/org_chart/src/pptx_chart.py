...
def _int(x):
    """Ensure an integer EMU value (PowerPoint XML requires ints)."""
    return int(x)
...
def _add_connector(slide, x1, y1, x2, y2, color=CONNECTOR_COLOR, thick_pt=CONNECTOR_THICK_PT):
    """
    Add a true straight connector line. All coordinates must be ints (EMUs).
    """
    x1 = _int(x1); y1 = _int(y1); x2 = _int(x2); y2 = _int(y2)
    shp = slide.shapes.add_connector(MSO_CONNECTOR.STRAIGHT, x1, y1, x2, y2)
    shp.line.color.rgb = RGBColor(*color)
    shp.line.width = Pt(thick_pt)
    return shp
...
def compute_layout(prs, max_cols, rows):
    ...
    if box_w < min_box_w:
        ...
        tenure_pt = max(TENURE_FONT_MIN_PT, TENURE_FONT_MIN_PT - 1)
...
def draw_department_slide(prs, dept_name, sub_df, page_num, total_pages, by_id_all):
    ...
    for row_idx, (is_ext, nodes) in enumerate(layout_rows):
        ...
        for col, node_id in enumerate(nodes):
            if is_ext:
                ...
                centers[node_id] = (_int(x + box_w//2), _int(current_top_y + box_h//2))
            else:
                r = by_id[node_id]
                x = x_for(col, cols)
                draw_person_box(
        ...
        current_top_y = _int(current_top_y + box_h + y_gap)
    ...
    def safe_bus_y(m_bottom_y, s_top_y):
        top = _int(m_bottom_y + GAP_SAFE)
        bottom = _int(s_top_y - GAP_SAFE)
        if bottom <= top:
            return _int((m_bottom_y + s_top_y) // 2)  # integer midpoint
        return _int((top + bottom) // 2)
    ...
    def bus_y_for(parent_id, m_bottom_y, s_top_y):
        ...
        return _int(max(top, min(bottom, y)))

    def draw_bus_connect(mx, m_bottom_y, sx, s_top_y, parent_id):
        by = bus_y_for(parent_id, m_bottom_y, s_top_y)

        # vertical down from manager to bus
        if abs(by - m_bottom_y) > 0:
            _add_connector(slide, mx, m_bottom_y, mx, by)

        # horizontal along the bus between manager and subordinate x
        if abs(sx - mx) > 0:
            _add_connector(slide, min(mx, sx), by, max(mx, sx), by)

        # vertical up from bus to subordinate
        if abs(s_top_y - by) > 0:
            _add_connector(slide, sx, by, sx, s_top_y)
    ...
    for mng, subs in children.items():
        if mng not in centers:
            continue
        mx, my = centers[mng]
        m_bottom_y = _int(my + box_w*0 + box_h//2)  # keep EMU math; +0 to emphasize int path
        for s in subs:
            if s not in centers:
                continue
            sx, sy = centers[s]
            s_top_y = _int(sy - box_h//2)
