...
st.write("Upload a **CSV or Excel (.xlsx/.xls)** with columns **employee_id, name, title, manager_id, department**. Optional: **tenure**, **tenure_calc** (numeric years).")
...
def draw_cover_slide(prs, dept_ordered, dept_counts, total_employees, page_num, total_pages, df):
    ...
    # Title
    title_shape = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.RECTANGLE, LEFT_M, Inches(0.2), prs.slide_width - LEFT_M - RIGHT_M, TITLE_H)
...
def get_department_order(df):
    ...
    if ORDER_COL in df.columns:
        first_order = (
            df.dropna(subset=[ORDER_COL])
              .assign(_ord=pd.to_numeric(df[ORDER_COL], errors="coerce"))[["department", "_ord"]]
              .dropna(subset=["_ord"]).groupby("department")["_ord"].first().to_dict()
        )
    else:
        first_order = {}
    ...
    for row_idx, (is_ext, nodes) in enumerate(layout_rows):
        ...
        for col, node_id in enumerate(nodes):
            if is_ext:
                ...
                x = x_for(col, cols)
                draw_person_box(slide, x, current_top_y, box_w, box_h,
                                r.name, r.title, getattr(r, "tenure", ""),
                                name_pt, title_pt, tenure_pt)
                centers[node_id] = (x + box_w/2, current_top_y + box_h/2)
...
def _try_preview(df: pd.DataFrame):
    try:
        st.subheader("Preview")
        st.dataframe(df.head(20))

        # ---- Department summary: Employees + Avg Tenure (if available) ----
        summary = None
        if "department" in df.columns and "employee_id" in df.columns:
            counts = (
        ...
        if "tenure_calc" in df.columns and summary is not None:
            tmp = df.copy()
            tmp["tenure_calc"] = pd.to_numeric(tmp["tenure_calc"], errors="coerce")
            avg_df = (
                tmp.groupby("department")["tenure_calc"].mean().round(1)
                   .reset_index(name="Avg Tenure (yrs)")
            )
            summary = summary.merge(avg_df, on="department", how="left")

        if summary is not None:
            st.write("Department summary (counts and optional average tenure):")
            st.dataframe(summary.sort_values("department"))
    except Exception as e:
        st.info(f"Preview note: {e}")
