...
def load_data(subfolder: str, test_column_header: str) -> pd.DataFrame:
    """
    ...
    5) Verifies that 'name_events' exists.
    """
    ...
    if test_column_header not in df.columns:
        raise KeyError(
            f"'name_events' column not found in loaded file; got {df.columns.tolist()}"
        )
