...
// SANCTION STATS PROCESS
const { execute_get_sanction_stats } = require('./src/slack_daily_stats/step_2_get_sanction_stats');
const { create_slack_message } = require('./src/slack_daily_stats/step_2a_create_sanction_message');
...
// Test endpoint
app.get('/sanction-test', async (req, res) => {
    console.log('/sanction-test route req.rawHeaders = ', req.rawHeaders);
...
app.post('/sanction-stats', async (req, res) => {
    // console.log('/sanction_stats route req.rawHeaders = ', req.rawHeaders);

    console.log('Received request for sanction - /sanction-stats :', {
        body: req.body,
        headers: req.headers,
        query: req.query,
        param: req.params,
    ...
    try {
        // Send a success response
        res.status(200).json({
            text: 'Retrieving sanction stats. Will respond shortly.',
        });
...
app.get('/scheduled-sanction-stats', async (req, res) => {
...
app.listen(PORT, async () => {
    ...
    // CLOUDFLARE TUNNEL
    console.log(`Tunnel using cloudflare https://usat-sanction.kidderwise.org/sanction-stats`)
