...
async function insert_data(pool, db_name, table_name, test = false) {
    ...
    for (let i = 0; i < loop_iteration; i++) {
        ...
        const query_to_insert_data = await step_4_create_participation_with_membership_match(start_date_time, end_date_time);

        console.log(`\nQuery Of ${i} of ${date_periods.length}: ****** Start Date: ${start_date_time}; End Date: ${end_date_time}.`)
