 ...
 *
 * Security model (everything here is reached only behind require_admin_auth in the server):
 ...
 *     for `node`. `npm` needs a shell on Windows only, and its args are fixed in the registry (no params).
 *   - params validated against their declared type/options; enum must match, int must be digits, date must
 *     be YYYY-MM-DD, path must stay inside the run dir (no `..`, no absolute). sql is passed as ONE argv
 *     element and is still guarded downstream by ask:sql's read-only guard.
 ...
 *
 * snake_case throughout (this file lives under admin/, which the lint doesn't scan, but the convention holds).
 */
...
const RUN_DIR = path.join(__dirname, '..');            // .../src/race_results_transform (where menu.js runs)
...
const audit = [];                                      // recent { at, id, action, params, status, code }
let seq = 0;
