...
- **sf_flow.spec.js** — the **SF Upload Queue** tab of the "Get Race Results" card (all `/api/sf/*` + `/api/login` stubbed; File System Access API disabled to force the server-folder fallback): From/To defaults + 14-day cap (From-anchored), inline sign-in + the Sign in/Sign out toggle, list → sort → auto-select → download (with progress) → Files queue table → click a row → convert → download, the 50-default/150-max cap (incl. the amber "raise Max files" glow), search filter, Cancel, and the "no files found" state. `.xls` case: with SheetJS **stubbed available** the `.xls` row is *not* flagged and there's no warning. Plus **Reload from disk**: download → open → convert → download (Downloaded), then the per-row **↻** re-reads via the stubbed `/api/sf/folder-file` and drops the row back to Converted.
- **slack_flow.spec.js** — the **Slack Ironman tab** (all `/api/slack/*` stubbed; File System Access API disabled to force the server-folder fallback): open the tab → the channel **picker auto-populates** from the bot's channels + the `/invite @bot` chip shows the real handle → List → newest auto-selected → download (server fallback) → Files queue → click a row → convert → download (Downloaded). Plus: switching the channel clears a stale list, and ↻ Refresh re-pulls the channel list.
- **folder_flow.spec.js** — the **From Folder tab** inside the "Get Race Results" card (the standalone folder card was removed). Opens the tab, drives the `webkitdirectory` fallback input directly (the native picker can't be scripted): pick 2 files → list/select in `#sfTable` → **Load** into the SAME Files queue (which shows File name · Modified, not Program/Owner) → open a row → convert → download (Excel) → status reaches Downloaded; plus search-filter, the **Max** cap (cap of 1 → 1 row checked) + Reset.
