...
async function format_markdown_table(row, month_name) {
  const markdown_message = [
    ...
    [""],
    [month_name + " '26", fmtCurrency(row.sales_rev_2025_actual), fmtNumber(row.sales_units_2025_actual), fmtRPU(row.sales_rpu_2025_actual)],
    [month_name + " '25", fmtCurrency(row.sales_rev_2024_actual), fmtNumber(row.sales_units_2024_actual), fmtRPU(row.sales_rpu_2024_actual)],
...
async function create_slack_message(result, type_input = "All", category_input = "All", month) {
  ...
  if (month === 'ytd') {
    ...
    if (month === undefined || month === null) {
      if (now.getDate() === 1) {
        // On the 1st, use the previous month (1-based, wrap Jan->Dec)
        month = now.getMonth() === 0 ? 12 : now.getMonth();
      } else {
        // Any other day, use current month (1-based)
