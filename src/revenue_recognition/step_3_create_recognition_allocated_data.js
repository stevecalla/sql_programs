...
const { step_2_query_rev_recognition_allocated_data } = require('../queries/rev_recognition/step_2_get_rev_recognition_allocation_data_050325');
...
async function execute_create_recognition_allocated_data() {
  const BATCH_SIZE   = 500;
  const TABLE_NAME   = 'rev_recognition_allocated_data';

  // GET QUERIES
  const CREATE_TABLE_QUERY = await query_create_rev_recognition_allocation_table(TABLE_NAME);
  const GET_DATA_QUERY = step_2_query_rev_recognition_allocated_data;
...
// execute_create_recognition_allocated_data().catch(err => {
//     console.error('Stream failed:', err);
//     process.exit(1);
// });

module.exports = {
  execute_create_recognition_allocated_data,
};
