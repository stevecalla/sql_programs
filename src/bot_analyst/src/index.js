...
/**
 * ✅ NEW (minimal): driver intent override.
 * If user clearly asks "why/drivers/variance/what caused", force driver_analysis.
 */
function is_driver_intent(user_text) {
  const s = String(user_text || "").toLowerCase();
  ...
  // also catch "vs last year" + "why/what drove" style phrasing
  const yoy_hint = s.includes("vs last year") || s.includes("vs. last year") || s.includes("year over year") || s.includes("yoy");

  const hit = patterns.some((p) => s.includes(p));
  return hit || (yoy_hint && (s.includes("why") || s.includes("drove") || s.includes("caused")));
}
...
app.event("app_mention", async ({ event, say }) => {
  ...
  try {
    ...
    const driver_override = is_driver_intent(text);

    for (let attempt = 1; attempt <= 2; attempt++) {
      plan = await plan_query_with_ai(text, { thread_ts, memory, domain });

      // ✅ NEW (minimal): override analysis_type if intent says "driver"
      if (driver_override && plan.analysis_type !== "driver_analysis") {
        console.log("🧷 driver intent override: forcing analysis_type=driver_analysis", {
          from: plan.analysis_type,
        });
        plan.analysis_type = "driver_analysis";
      }
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        // ✅ keep override applied on replan too
        if (driver_override && plan.analysis_type !== "driver_analysis") {
          console.log("🧷 driver intent override (replan): forcing analysis_type=driver_analysis", {
            from: plan.analysis_type,
          });
          plan.analysis_type = "driver_analysis";
        }
    ...
    // 4) Senior analyst narrative layer (+ no-evidence gating)
      ...
      try {
        narrative = await analyze_results_with_ai({
          ...
          notes: {
            ...
            zero_guard,
            driver_override_applied: driver_override,
          },
        });
      } catch (e) {
