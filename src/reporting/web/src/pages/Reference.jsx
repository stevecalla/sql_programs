...
const REF = [
  ...
  { title: 'Penetration & Opportunity (per-capita)', note: 'Per-capita metrics use US Census population loaded by step 2c. The exact population source + vintage appears in the population/penetration metric tooltips.', items: [
    ['Population (Census)', 'State resident population — US Census ACS 1-year. Primary source is the Census API (needs CENSUS_API_KEY); falls back to BigQuery public census (2021) if no key. Loaded into census_state_population; the vintage is stamped in its source column.'],
    ['Adult participation / 1,000 pop', 'Adult participations at in-state events ÷ population × 1,000 — supply-side per-capita reach (where events are held).'],
    ['Home penetration / 1,000 pop', 'Distinct adult residents who race ÷ population × 1,000 — demand-side reach, counted once whether they race at home or away. National ≈ 0.44. Counted live via /api/home (by member_state_code_addresses, adult only).'],
    ['Home vs event penetration', 'Event = where races happen (locals + visitors); Home = where racers live. The gap flags feeders (residents race out of state) vs destinations (host more than their residents race).'],
    ['Known-member basis', 'Home penetration counts member-matched residents only, so it is directional (undercounts non-member finishers); the ranking/spread is robust.'],
    ['Opportunity view — bands', 'Four bands — Leader / Mid / Under-penetrated / Floor — set three ways. National-relative (default): Leader ≥ the national rate, Floor ≤ half national (Mid empty). Statistical: from the distribution of state penetration values — Quantile (Leader = top 20%, Floor = bottom 20%, split at the median) or Std-dev (Leader ≥ mean+σ, Floor ≤ mean−σ, split at the mean; σ multiplier adjustable). Absolute: fixed cutoffs you type. The national benchmark is population-weighted (Σ resident athletes ÷ Σ population × 1,000).'],
    ['Opportunity view — headroom', 'Estimated additional resident athletes to lift a state to the national rate = (national − state penetration) ÷ 1,000 × state population. The ranking panel sorts states by headroom (biggest real-world targets first); zero for states already at/above national.'],
  ] },
