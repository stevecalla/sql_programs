...
const REF = [
  ...
  { title: 'Penetration & Opportunity (per-capita)', note: 'Per-capita metrics use US Census population loaded by step 2c. The exact population source + vintage appears in the population/penetration metric tooltips.', items: [
    ...
    ['National /1k', 'The population-weighted national benchmark for each measure: Σ numerator ÷ Σ population × 1,000 over the 50 states + DC (big states pull the mean correctly). All-states and in-state have their own national rate.'],
    ['Jurisdiction scope (who counts)', 'Penetration counts only residents whose home state is one of the 50 states + DC — the jurisdictions that have a Census population denominator (member_state_code_addresses IN the region_data set; a territory carries only if it has Census population). Member-matched adult racers with foreign, military (APO/FPO), or invalid home codes — roughly 12% — are EXCLUDED, because there is no population to divide by (a rate needs a denominator). This is why the penetration national count (~145k) sits below the raw all-member count (~165k): the ~20k difference is home codes outside the 50 states + DC.'],
