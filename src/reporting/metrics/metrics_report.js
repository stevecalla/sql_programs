'use strict';
// Usage-analytics aggregation for the reporting app. Returns the shared "report contract"
// (structured `data` + human `sections`) consumed by the React /metrics dashboard. Modeled on
// src/salesforce_merge/metrics/metrics_report.js but computes ONLY the reporting-applicable blocks
// (no merge / restore / build / funnel / sandbox-vs-prod blocks — reporting has no such events).
//
// The reporting_events table uses a single `ts` DATETIME (server local time) — there is no separate
...
async function build_report(pool, opts) {
  ...
  const report = {
    title: 'USAT Reporting — Metrics (last ' + days + ' days)',
    range: 'app=' + APP,
    sections: [
      { heading: 'Usage', lines: [
...
// Table size — computed directly (retention.size() assumes created_at_*, which reporting lacks).
