...
// Flat analytics columns the /metrics dashboard aggregates. Generic reporting-applicable subset of
// the merge tool's set — no merge-only domain columns (mode / set_count / account_count / …).
const EXTRA_COLUMNS = [
...
async function ensure(pool) {
  ...
  for (const [name, ddl] of EXTRA_COLUMNS) {
    try {
      await p.query('ALTER TABLE reporting_events ADD COLUMN IF NOT EXISTS ' + name + ' ' + ddl);
    } catch (e) {
      try { await p.query('ALTER TABLE reporting_events ADD COLUMN ' + name + ' ' + ddl); }
      catch (e2) { /* already exists (dup column) — fine */ }
    }
  }
  try { await p.query('ALTER TABLE reporting_events ADD INDEX ix_visitor (visitor_id)'); }
...
async function log(evt) {
  try {
    ...
    await pool.query(
      'INSERT INTO reporting_events (app, event_name, actor, role, panel, is_test, view, filter_name, ' +
      ...
      ['reporting', String(e.event_name || 'event'), e.actor || null, e.role || null, e.panel || null,
