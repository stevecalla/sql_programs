...
const SECTIONS = [
  ...
  { label: 'DATA PIPELINE (ETL)', color: YELLOW, items: [
    { id: 8, label: 'Create ZIP code table (step 2b)', desc: 'MySQL: rebuild zip_lat_lng_reference (ZIP → lat/lng/city/state/county) from BigQuery public data', node: ['src/participation_data/step_2b_load_zip_reference.js'], cli: 'node src/participation_data/step_2b_load_zip_reference.js' },
    { id: 9, label: 'Build participation summary (step 3i)', desc: 'MySQL: rebuild summary + flows + events tables from the base data', node: ['src/participation_data/step_3i_create_participation_summary.js'], cli: 'node src/participation_data/step_3i_create_participation_summary.js' },
    { id: 10, label: 'Load metrics to BigQuery (step 3j)', desc: 'Upload summary / flows / events tables to BigQuery (WRITE_TRUNCATE)', node: ['src/participation_data/step_3j_load_bq_participation_summary_metrics.js'], cli: 'node src/participation_data/step_3j_load_bq_participation_summary_metrics.js' },
  ]},
  { label: 'TESTING', color: CYAN, items: [
    { id: 11, label: 'Unit tests (node:test)', desc: 'Status, auth flow, bootstrap gating — no DB needed', run: 'reporting_test', cli: 'npm run reporting_test' },
  ]},
  { label: 'SERVER & USERS', color: CYAN, items: [
    { id: 12, label: 'Add / update a user', desc: 'Create a web-app login (username, password, role)', node: ['src/reporting/admin.js', 'add'], cli: 'node src/reporting/admin.js add' },
    { id: 13, label: 'List users', desc: 'Show .env recovery + stored web-app logins', node: ['src/reporting/admin.js', 'list'], cli: 'node src/reporting/admin.js list' },
    { id: 14, label: 'Reset a user password', desc: 'Set a new password for an existing login (hashed)', node: ['src/reporting/admin.js', 'passwd'], cli: 'node src/reporting/admin.js passwd' },
    { id: 15, label: 'Remove a user', desc: 'Delete a stored login (prompts + confirm)', node: ['src/reporting/admin.js', 'remove'], cli: 'node src/reporting/admin.js remove' },
    { id: 16, label: 'Show panel access', desc: 'Print the default + per-user panel allow-list', node: ['src/reporting/admin.js', 'access'], cli: 'node src/reporting/admin.js access' },
    { id: 17, label: 'Auth + access tests', desc: 'auth_store roles, .env recovery, panel access', node: ['--test', 'src/reporting/tests/auth.test.js'], cli: 'node --test src/reporting/tests/auth.test.js' },
  ]},
  { label: 'OPEN', color: GREEN, items: [
    { id: 18, label: 'Open dev UI', desc: 'Vite dev server (hot reload) at :5174', open: 'http://localhost:5174', cli: 'open http://localhost:5174' },
    { id: 19, label: 'Open built UI', desc: 'Production-style single-port app at :8021', open: 'http://localhost:8021', cli: 'open http://localhost:8021' },
    { id: 20, label: 'Open via proxy (/reporting)', desc: 'The app through the proxy at :8000/reporting/', open: 'http://localhost:8000/reporting/', cli: 'open http://localhost:8000/reporting/' },
    { id: 21, label: 'Check API status', desc: 'Ping the backend health endpoint', status: true, cli: 'curl http://localhost:8021/api/status' },
  ]},
  { label: 'PM2 (production)', color: RED, items: [
    { id: 22, label: 'pm2 start', desc: 'Run the server under pm2 (production)', run: 'pm2_start_reporting', cli: 'npm run pm2_start_reporting' },
    { id: 23, label: 'pm2 restart', desc: 'Restart the pm2 process', run: 'restart_reporting', cli: 'npm run restart_reporting' },
    { id: 24, label: 'pm2 stop', desc: 'Stop the pm2 process', run: 'stop_reporting', cli: 'npm run stop_reporting' },
    { id: 25, label: 'pm2 logs', desc: 'Tail the pm2 logs', run: 'pm2_logs_reporting', cli: 'npm run pm2_logs_reporting' },
  ]},
];
