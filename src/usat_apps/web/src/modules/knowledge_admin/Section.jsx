...
// queue share: retrieval blend (keyword <-> semantic), the embedding model + reindex, the AI model registry,
// queue access, and the web allowlist. Admin-only; talks to the neutral /api/knowledge-admin/* endpoints.
// Every section is a collapsible card (collapsed by default) with a "What this controls" note.
...
const S = {
  card: { background: 'var(--card,#fff)', border: '1px solid var(--line,#e4e7ec)', borderRadius: 12, padding: '14px 18px', marginBottom: 12 },
  head: { display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', userSelect: 'none' },
  h2: { fontSize: 15, margin: 0, flex: 1 },
...
// Collapsible card via the SHARED Collapsible component (lib/ui.jsx), styled with ka-* classes. Collapsed by
// default; each card owns its open state. Same common component the email queue and chatbot cards use.
...
// ---------- Retrieval + embeddings + allowlist (share one fetch) ----------
function RetrievalCards() {
  ...
  const load = () => req('/settings').then((r) => { setData(r); setWeight(Math.round((r.settings.retrieval_weight || 0) * 100)); }).catch((e) => setErr(e.message));
  useEffect(() => { load(); /* eslint-disable-next-line */ }, []);

  const st = data && data.settings, status = data && data.status, allow = (data && data.allowlist) || [];
  ...
  const addHost = () => { const h = host.trim(); if (!h) return; saveAllow(allow.concat([h])); setHost(''); };
  ...
  if (err && !data) return <div style={S.card}><b>Couldn’t load retrieval settings.</b><div style={S.err}>{err}</div></div>;
  if (!data) return <div className="loading" style={{ padding: 12 }}>Loading…</div>;
  ...
  return (
    <>
      {err ? <div style={{ ...S.note, color: '#dc2626', background: '#fef2f2', borderColor: '#fecaca' }}>{err}</div> : null}
        ...
        <div style={S.demo}>
          {ranked.map((r, i) => (
            <div key={i} style={{ background: 'var(--card,#fff)', border: '1px solid var(--line,#e4e7ec)', borderRadius: 9, padding: '9px 11px', marginBottom: i < ranked.length - 1 ? 8 : 0 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}><span style={{ fontWeight: 600, fontSize: 13 }}>{r.x.t}</span><span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--dim,#6b7280)' }}>#{i + 1}</span></div>
              <div style={{ ...S.muted, fontSize: 11 }}>{r.x.c}</div>
              <div style={{ display: 'grid', gridTemplateColumns: '62px 1fr 40px', gap: '5px 8px', alignItems: 'center', marginTop: 7, fontSize: 11, color: 'var(--dim,#6b7280)' }}>
                <span>keyword</span>{bar(r.kn * 100, '#8b5cf6')}<span style={{ textAlign: 'right' }}>{r.kn.toFixed(2)}</span>
                <span>semantic</span>{bar(r.sn * 100, '#0ea5e9')}<span style={{ textAlign: 'right' }}>{r.sn.toFixed(2)}</span>
                <span style={{ color: '#3b82f6', fontWeight: 600 }}>blended</span>{bar(r.b * 100, '#3b82f6')}<span style={{ textAlign: 'right', color: '#3b82f6', fontWeight: 600 }}>{r.b.toFixed(2)}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <Collapse title={<>Embedding model<Applies /></>} summary={st.embeddings_enabled ? st.embedding_model : 'off'}>
      ...
      <Collapse title={<>Web allowlist<Applies /></>} summary={allow.length + ' host' + (allow.length === 1 ? '' : 's')}>
...
// ---------- AI model registry (migrated from the email admin) ----------
...
// ---------- Queue access (migrated from the email admin) ----------
function QueueAccess() {
  ...
  const load = () => req('/queue-access').then((r) => { setData(r); applyDefault(r.access || { default: 'all', users: {} }); const u = (r.users || [])[0] || ''; setSelUser(u); applyUser(u, r.access || {}); }).catch((e) => setErr(e.message));
  useEffect(() => { load(); /* eslint-disable-next-line */ }, []);
...
export default function KnowledgeAdminSection() {
  return (
    <div style={{ maxWidth: 900, padding: '18px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap', marginBottom: 10 }}>
        <div style={{ flex: 1, minWidth: 240 }}>
          ...
          <div style={{ ...S.muted, marginTop: 2 }}>The one place for the settings the chatbot and the email queue share. Expand a section to see what it controls.</div>
        </div>
      ...
      <RetrievalCards />
      <ModelRegistry />
      <div style={{ ...S.muted, fontSize: 11.5, textAlign: 'center', marginTop: 6 }}>Shared config key <code>knowledge</code> · endpoints <code>/api/knowledge-admin/*</code> · admin only</div>
    </div>
  );
}
