...
// "Ask your data" — a faithful React port of the metrics Ask box, wired to the salesforce_email_queue
// backend (api.metricsAsk / metricsAskModels / metricsAskCorrect): model picker, raw-SQL toggle,
// follow-up conversation threads, suggestion chips, natural-language answers with a result table and
// collapsible SQL, and per-answer "Correct this" feedback. Email-queue-local copy so it talks to its
// own endpoints rather than merge's.
const SUGGESTIONS = [
  'How many AI calls per operator in the last 30 days?',
  'Which queues have the most activity, in a table?',
  'Replies copied by day?',
  'Which AI models cost the most?',
  'Most recent active operators, in a table?',
  'Sends and status changes per day?',
];
  ...
  return (
    <div>
      <div className="mx-ask-box">
        <textarea ref={taRef} className="mx-ask-q" rows={1} value={q} placeholder={sqlMode ? 'Read-only SELECT over the email-queue events…' : 'Ask about the usage data…  e.g. AI calls by operator last week'}
