...
function SectionBlock({ s }) {
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      {title && <h3 style={{ margin: '6px 0 2px', fontSize: 14 }}>{title}</h3>}
      {lines.map((l, i) => <p key={i} className="muted small" style={{ margin: '2px 0' }}>{typeof l === 'string' ? l : JSON.stringify(l)}</p>)}
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div className="mx-cards">
        ...
        <Card k="AI cost" v={usd(ai.cost_usd, 2)} s={fmt(ai.corrections_used) + ' corrections used'} />
      ...
      {Array.isArray(rep.sections) && rep.sections.length > 0 && (
        <div className="mx-panel">
          <h2>Summary <span className="dim" style={{ fontWeight: 400, fontSize: 13, textTransform: 'none', letterSpacing: 0 }}>{rep.range || ''}</span></h2>
      ...
      {/* activity by day */}
      <ChartCard id="chart_days" title="Activity by day — visits · threads · AI calls · acks" type="multibar" theme={theme}
        labels={dayLabels}
        series={[
        ...
        headers={['Day', 'Visits', 'Threads', 'AI calls', 'Acks']}
        rows={d.by_day.map((r) => [r.day, r.visits, r.threads, r.ai_calls, r.acks])} />
      ...
      <div className="mx-grid2">
        ...
        <ChartCard id="chart_dow" title="Activity by day of week" theme={theme} color={COL.acks}
      ...
      {/* AI provider / verdict */}
      <div className="mx-grid2">
        <ChartCard id="chart_provider" title="AI by provider" theme={theme} color={COL.ai}
          labels={d.by_provider.map((p) => p.provider)} values={d.by_provider.map((p) => p.n)}
          headers={['Provider', 'Calls', 'Avg ms']} rows={d.by_provider.map((p) => [p.provider, p.n, p.avg_ms])} />
      ...
      <div className="mx-grid2">
        ...
        <ChartCard id="chart_model" title="AI by model — calls (cost in table)" theme={theme} color={COL.green}
          labels={d.by_model.map((m) => m.model)} values={d.by_model.map((m) => m.n)}
          headers={['Model', 'Calls', 'Prompt tok', 'Compl tok', 'Cost']}
          rows={d.by_model.map((m) => [m.model, m.n, m.prompt_tokens, m.completion_tokens, usd(m.cost_usd, 4)])} />
      </div>

      {/* AI spend — real vs test vs total, and by env */}
      <div className="mx-panel">
        <h2>AI spend</h2>
        <div className="mx-cards">
          <Card k="Real spend" v={usd(spend.real_usd, 2)} s={fmt(spend.real_calls) + ' calls · ' + fmt(spend.real_tokens) + ' tok'} />
          <Card k="Test spend" v={usd(spend.test_usd, 2)} s={fmt(spend.test_calls) + ' calls · ' + fmt(spend.test_tokens) + ' tok'} />
          <Card k="Total spend" v={usd(spend.total_usd, 2)} s="real + test" />
        </div>
      </div>
      ...
      {/* by queue */}
      <ChartCard id="chart_queue" title="Activity by queue — events (AI calls / threads in table)" theme={theme} color={COL.threads}
        labels={d.by_queue.map((q) => q.queue)} values={d.by_queue.map((q) => q.events)}
        headers={['Queue', 'Events', 'AI calls', 'Threads']} rows={d.by_queue.map((q) => [q.queue, q.events, q.ai_calls, q.threads])} />
      ...
      <div className="mx-grid2">
        <div className="mx-panel">
          ...
          <table>
            <thead><tr><th className="mx-rn">#</th><th>Actor</th><th>AI calls</th><th>Threads</th><th>Events</th><th>Last seen</th></tr></thead>
            <tbody>
              {d.top_operators.length === 0 && <tr><td className="dim" colSpan={6}>none</td></tr>}
              {d.top_operators.map((o, i) => (<tr key={o.actor + i}><td className="mx-rn">{i + 1}</td><td>{o.actor || '—'}</td><td>{fmt(o.ai_calls)}</td><td>{fmt(o.threads)}</td><td>{fmt(o.events)}</td><td>{formatMtn(o.last_seen)}</td></tr>))}
            </tbody>
          </table>
        </div>
        <div className="mx-panel">
