...
const SECTIONS = [
    ...
    body: (
      <>
        ...
        <div className="defs">
          ...
          <div className="defs-row"><span className="defs-term lg">Groups, accounts &amp; "mergeable"</span><span className="defs-body">A <strong>merge-id group</strong> is every account that shares one merge ID; its <strong>size</strong> is that account count, and every account belongs to exactly one group. Because of that, <strong>accounts and groups reconcile across the Dashboard, Select Merges, and Merge Ops</strong> — for any given bucket, both numbers match panel to panel. A group of <strong>size 1 is a singleton</strong> (a merge ID on a single account — nothing to merge); <strong>"mergeable" means size ≥ 2</strong>, which you isolate with the <strong>Size ≥ 2</strong> filter (Size = 1 shows the singletons). The batch sampler only ever draws from mergeable (2+) sets. Every filter dropdown shows a <strong>live count per option</strong>, scoped to the other active filters (like the Size dropdown).</span></div>
        </div>
      </>
    ),
  },
    ...
    body: (
      <>
        ...
        <div className="defs">
          ...
          <div className="defs-row"><span className="defs-term lg">Activity relationships (Tasks/Events)</span><span className="defs-body">Salesforce manages activity links itself: <code>Task.AccountId</code> is read-only and the <code>TaskRelation</code>/<code>TaskWhoRelation</code> junctions can’t be updated at all — so an update against them is <strong>always</strong> rejected (that’s the <em>“Unable to create/update AccountId”</em> / <em>“entity type cannot be updated”</em> messages). These aren’t real failures. The tool now <strong>excludes them from the pre-merge snapshot and skips them cleanly on restore</strong>, so those false errors no longer appear. Salesforce keeps the activities on the survivor; they aren’t reattached to the loser.</span></div>
          <div className="defs-row"><span className="defs-term lg">Master-detail children (data-loss check)</span><span className="defs-body">Salesforce <strong>re-parents lookup children</strong> to the survivor during a merge, but <strong>master-detail children cascade-delete</strong> with the loser. If such a child isn’t in the pre-merge snapshot, restore can’t bring it back and it purges at ~15 days. To audit a specific object (e.g. “Donor Gift Summary”), run the read-only probe — <strong>merge menu items 34/35</strong> (<code>probe_donor_gift.js</code>): it reports master-detail vs lookup, whether the <em>integrated user can even see the object</em>, and finds a foundation/donor duplicate to test. If the integrated user lacks read on the object, restore can never recover it — a permissions fix, not a code fix. <strong>Confirmed in sandbox:</strong> <code>DonorGiftSummary</code> is <strong>Master-Detail(Account) (Unique)</strong> — a per-account giving rollup (derived; the real gifts live in <em>Gift Transaction</em>). The integrated user currently can’t see it, so grant Read + Delete and Salesforce should cascade-restore it when the loser is undeleted.</span></div>
          <div className="defs-row"><span className="defs-term lg">Managed-package fields (field-level security)</span><span className="defs-body">some child records point back to a Contact/Account through a <strong>managed-package lookup field</strong> — e.g. iWave’s <code>iWave_PROscores__Contact__c</code> on <em>iWave PRO Score</em> (<code>iWave_PROscores__iWave_PRO_Score__c</code>). If the write user lacks <strong>field-level Edit</strong> on that field, restore can’t re-point the child back and skips it cleanly with <em>“field not writable for this user, needs field-level edit (FLS)”</em>. This is a <strong>permissions</strong> gap, not a code bug — and it affects <em>restore only</em>: the merge itself is fine (Salesforce’s native merge re-parents these onto the survivor, the intended result) and the record is never deleted, just left on the survivor. Grant the write user field-level Edit on the field to make restore fully reversible. Same category applies to <code>AccountContactRelation</code>, whose <code>ContactId</code> is read-only and system-managed — the tool now skips it because Salesforce recreates that relation automatically when the Contact’s <code>AccountId</code> is restored.</span></div>
  ...
  {
    title: "Parallel workers — running big jobs faster (planned)",
    text: "parallel workers running big jobs faster (planned) a large merge job is automatically split into several smaller parallel batches that a cluster of background workers drains side by side, so a big run finishes several times faster. it is transparent to end users — you just see one job with one progress bar that goes faster; there is no parallelism setting a normal user can change. auto-split when you run a job the tool chunks the approved sets into batches (default 5 sets per batch, admin-adjustable) and enqueues them as one job; whatever workers are online pick them up together. a job is a group of parallel batch-runs sharing a job id; small jobs stay a single run — identical to the non-parallel path. workers active the progress bar shows how many workers are actively merging, and aggregates completed sets across all the batches; cancel stops the whole job. safety is unchanged the queue claim is atomic (no set is ever run twice), lock-retry absorbs contention, and simulate/execute gates + typed confirmation still apply. async apex spends the same total, just faster, so the pre-flight budget still gates a run. admin-only merge ops panel (users & access grantable) an admin panel — access granted per user in the users & access section — adds three things: live settings (turn parallel on/off as a kill switch, set chunk size and max sets per job, set the worker count) without touching env vars or redeploying; a live workers view (workers online, queue depth, per-worker split); and a batch-run control to launch a parallel run against approved sets straight from the ui. worker count is adjustable on the fly (scales the cluster) and the app is cluster-agnostic — it chunks the job and whatever workers exist drain it. reliable metrics throughput (merges per minute) and seconds per merge (median batch) are the trustworthy speed numbers; the org-wide api/apex counters are daily-headroom context, not per-merge precision.",
    body: (
      <>
        <p>
          A large merge job is <strong>automatically split into several smaller parallel batches</strong>{" "}
          that a cluster of background workers drains side by side, so a big run finishes several times
          faster. It is <strong>transparent</strong> to end users — you just see one job with one progress
          bar that goes faster; there is <em>no parallelism setting a normal user can change</em>.
        </p>
        <div className="defs">
          <div className="defs-row"><span className="defs-term lg">Auto-split into a job</span><span className="defs-body">when you run a job the tool <strong>chunks the approved sets</strong> into batches (default <strong>5 sets/batch</strong>, admin-adjustable) and enqueues them as one <em>job</em>; whatever workers are online pick them up together. A <strong>job</strong> is a group of parallel batch-runs sharing a <code>job_id</code>. Small jobs stay a single run — <strong>identical to the non-parallel path</strong> (behavior-preserving).</span></div>
          <div className="defs-row"><span className="defs-term lg">Workers active</span><span className="defs-body">the progress bar shows <strong>how many workers are actively merging</strong> and aggregates completed sets across all the batches into one bar + ETA. <strong>Cancel</strong> stops the whole job (every batch), at the next set boundary.</span></div>
          <div className="defs-row"><span className="defs-term lg">Safety is unchanged</span><span className="defs-body">the queue claim is <strong>atomic</strong> (no set is ever run twice), lock-retry absorbs transient contention from shared rollups (dlrs/Cirrus), and the <em>Simulate/Execute</em> gates + typed confirmation still apply. Async Apex spends the <em>same total</em>, just faster, so the pre-flight budget still gates a run.</span></div>
          <div className="defs-row"><span className="defs-term lg">Admin-only Merge Ops panel</span><span className="defs-body">an admin panel — <strong>access granted per user in the Users &amp; Access section</strong> — adds three things: <strong>live settings</strong> (turn parallel on/off as a kill switch, set chunk size and max sets per job, set the worker count) with <em>no env vars or redeploy</em>; a <strong>live workers view</strong> (workers online, queue depth, per-worker split); and a <strong>batch-run control</strong> to launch a parallel run against approved sets straight from the UI.</span></div>
          <div className="defs-row"><span className="defs-term lg">Worker count on the fly</span><span className="defs-body">the cluster size is <strong>adjustable from the panel</strong> (it scales the worker cluster), and the app is <em>cluster-agnostic</em> — it chunks the job and whatever workers exist drain it. Target <strong>4 workers</strong>, tune to taste.</span></div>
          <div className="defs-row"><span className="defs-term lg">Merge Ops panel (admin)</span><span className="defs-body">an admin-only page (grant <em>Merge Ops</em> in Users &amp; Access) with tabs for <strong>Settings</strong> (live parallel/chunk/max-batch/worker/Apex-cap — no redeploy) and <strong>Batch run</strong> (launch a job against the approved queue, or a filtered random sample — same Foundation/tier/signal/size filters as Select Merges — with an optional Restore phase), plus an always-visible <strong>Workers</strong> view with on-the-fly <code>pm2 scale</code> and a live <code>pm2 logs</code> tail.</span></div>
          <div className="defs-row"><span className="defs-term lg">Async-Apex cap (auto-pause)</span><span className="defs-body">a job automatically <strong>pauses</strong> (its remaining batches held, in-flight set finishes) when async-Apex usage reaches the configurable cap, and is <strong>resumable</strong> from Process Merges or the Merge Ops panel — so a big parallel burn can&apos;t blow the 250k daily limit. Already-merged sets drop out on resume via the drift check.</span></div>
          <div className="defs-gate">Trustworthy speed numbers are <strong>throughput (merges/min)</strong> and <strong>seconds per merge (median batch)</strong> — concurrency-proof. The org-wide API/Apex counters are daily-headroom context, not per-merge precision.</div>
        </div>
      </>
    ),
  },
    ...
    body: (
      <>
        ...
        <div className="defs">
          ...
          <div className="defs-row"><span className="defs-term lg">Un-approve (move back to queued)</span><span className="defs-body">an approved set can be pulled back to <em>queued</em> with the <strong>↩ Move to queued</strong> button on both Select Merges and Process Merges. It drops off the approved (processable) list until you re-approve it — so you can park a set for re-review or edits without deleting it or its stage baseline. Guarded to <em>approved → queued</em> only.</span></div>
          <div className="defs-row"><span className="defs-term lg">Batch limit per run</span><span className="defs-body">Process Merges has a <strong>Max sets per run</strong> control (editable 1–500, default from <code>MERGE_MAX_BATCH</code>) that caps how many sets one <em>Execute</em> processes. Enforced <strong>server-side</strong> (hard ceiling 500) so a run can never fire an unbounded batch; Simulate is unlimited.</span></div>
    ...
    body: (
      <>
        ...
        <div className="defs">
          ...
          <div className="defs-row"><span className="defs-term lg">App Recycle Bin ≠ SF Recycle Bin</span><span className="defs-body">the <strong>App Recycle Bin</strong> panel lists every Account the API returns as <code>IsDeleted=true</code> (the <code>queryAll</code>/scanAll endpoint) — a <strong>superset</strong> of the actual Salesforce Recycle Bin. A record moves through four states: <strong>(1) live</strong> (<code>IsDeleted=false</code>); <strong>(2) in the SF Recycle Bin</strong> (<code>IsDeleted=true</code>, restorable — lasts ~15 days <em>or until the bin overflows its size cap</em>, since Salesforce auto-purges oldest first, so a busy sandbox can lose recent records within hours); <strong>(3) purged, physical-delete pending</strong> (gone from the SF bin, <code>undelete</code> fails “not in recycle bin”, but <code>queryAll</code> still returns <code>IsDeleted=true</code> during a lag window — <em>these appear in the panel but are not restorable</em>); <strong>(4) physically gone</strong> (<code>queryAll</code> returns nothing; the row drops off on its own). <strong>States 2 and 3 are indistinguishable to any query</strong> — Salesforce exposes no “in the bin” field, only <code>IsDeleted</code> — so the panel can’t pre-filter to only restorable rows; the definitive test is attempting the restore (a state-3 set auto-routes to recreate-from-backup). Verify in <strong>Workbench → SOQL Query</strong> with <strong>“Deleted and archived records: Include”</strong> ticked — and note SOQL has no <code>SELECT *</code>, you must name fields: <code>SELECT Id, Name, IsDeleted, MasterRecordId, LastModifiedDate FROM Account WHERE Id IN (…)</code>.</span></div>
          ...
          <div className="defs-row"><span className="defs-term lg">All-or-nothing eligibility</span><span className="defs-body">a set is flagged <strong>✓ restorable</strong> only if <em>every</em> loser in it still reads <code>IsDeleted=true</code>. If even one is gone (purged or already restored), the whole set shows <strong>✕ expired</strong> and is skipped; <strong>— unknown</strong> means the eligibility check couldn’t reach Salesforce. <em>Caveat:</em> that check is a <code>queryAll</code> read, so it can’t see a <strong>state-3</strong> purge (record still <code>IsDeleted=true</code> but no longer in the bin) — such a set may show <strong>✓</strong> yet flip to recreate-from-backup when the <code>undelete</code> is actually attempted and Salesforce rejects it.</span></div>
          ...
          <div className="defs-row"><span className="defs-term lg">Post-merge edit gate</span><span className="defs-body">a merge captures a <strong>post-merge snapshot</strong> of the survivor (its values + Salesforce’s <code>LastModifiedDate</code>) the moment it finishes. On the Restore page, <strong>Check post-merge changes</strong> compares that against the survivor’s current state to flag any set whose survivor was <strong>edited in Salesforce after the merge</strong> (⚠ <em>edited since merge</em> vs ✓ <em>untouched</em>), showing both dates and the changed fields. On <strong>Execute</strong>, flagged sets are <strong>held</strong> — left in the queue, not restored — unless you tick the acknowledgment, so a blind restore can’t silently overwrite a legitimate later edit. Sets merged before this feature show <em>no baseline</em> and aren’t gated.</span></div>
          ...
          <div className="defs-row"><span className="defs-term lg">History page</span><span className="defs-body">the <strong>History</strong> tab is a searchable, sortable, exportable view of that audit — every processed set across merge / restore / recreate / simulate / skip / fail. Filter by result, search by survivor name / account id / source, expand a row for the field-level diff (drift, or restore/recreate reset + kept), and use the <strong>📎 dossier</strong> link to download the archived workbook for that action.</span></div>
          <div className="defs-row"><span className="defs-term lg">Lifecycle stamp (usat_was_*)</span><span className="defs-body">every action re-stamps the survivor’s three optional Account fields: <code>usat_was_merged__c</code> (checkbox: <strong>true</strong> after a merge, <strong>false</strong> after a restore/recreate — i.e. “is this currently a merge product?”), <code>usat_was_merged_date__c</code> (the last-action date), and <code>usat_was_merged_by__c</code> (text: <strong>“MERGE — you”, “RESTORE — you”, or “RECREATE — you”</strong>). Controlled per run by the <strong>Stamp survivor</strong> checkbox on <strong>Process Merges, Restore, and Recreate</strong> (default on), or disable globally with <code>MERGE_STAMP_SURVIVOR=false</code>. Best-effort and per-field: the tool <strong>never creates these fields</strong>, and if they don’t exist (or a write is rejected) it just logs “stamp skipped” — it <strong>never fails the merge/restore/recreate</strong>.</span></div>
          <div className="defs-row"><span className="defs-term lg">Merge dossier</span><span className="defs-body">each merge/restore/recreate builds a multi-tab Excel — <em>Summary · Reference (with the SQL/SOQL to pull each piece, incl. the file itself) · Overrides &amp; survivorship · Stage baseline · Pre-merge snapshot · Post-merge snapshot · Children re-parented · History &amp; drift</em> (the three snapshots capture the set at staging → pre-merge → post-merge) — and attaches it as a <strong>Salesforce File</strong> (one file, linked to many records): a <strong>merge</strong> attaches to the survivor; a <strong>restore/recreate</strong> attaches to every affected record (survivor + restored/recreated accounts + re-pointed children). A copy is kept in the tool’s DB and linked on the History row. Toggle per run with the <strong>📎 Attach dossier</strong> checkbox on <strong>Process Merges, Restore, and Recreate</strong> (default on, each remembers your choice); disable globally with <code>MERGE_ATTACH_DOSSIER=false</code>. Best-effort — a dossier problem never blocks the action.</span></div>
          <div className="defs-row"><span className="defs-term lg">File shares on restore</span><span className="defs-body">Salesforce <strong>file shares</strong> (<code>ContentDocumentLink</code>) can’t be re-parented with a field update — they are insert/delete only. So on restore/recreate the tool <strong>additively re-links</strong> a share — it creates a new one on the loser (from the captured <code>ContentDocumentId</code>) and <strong>keeps the survivor’s</strong>, so a file is never unshared or lost. Shares captured before this feature have no <code>ContentDocumentId</code>, so they’re left on the survivor with a clear note; the <code>repair_file_shares.js</code> tool re-links those. In History, rows carrying this note show an <strong>ⓘ file share</strong> hint explaining it.</span></div>
    ...
    body: (
      <>
        ...
        <div className="defs">
          ...
          <div className="defs-row"><span className="defs-term lg">Async Apex — the tighter limit</span><span className="defs-body">a merge isn’t only REST calls: it <strong>triggers async Apex</strong> — Account triggers, managed packages, and rollup engines (e.g. <code>dlrs</code>, Cirrus) that recompute gift rollups when accounts merge. That spends <strong>DailyAsyncApexExecutions</strong> (~250K/day), a limit <strong>far smaller</strong> than Daily API Requests (millions), so on a big bulk merge <em>it</em> — not the API budget — runs out first. The tool now measures async-Apex cost per run (stress report + the <em>Apex cost</em> column on Recent runs) and captures it on every live refresh so it builds a trend, alongside Bulk API batches (which the full data pull spends).</span></div>
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <p>On the <strong>History</strong> page, each processed set shows two figures:</p>
        <div className="defs">
