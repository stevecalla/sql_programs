...
const SECTIONS = [
  ...
  { label: 'TESTING', color: CYAN, items: [
    ...
    { id: 10, label: 'Salesforce merge tests', desc: 'Just the salesforce_merge module (api, queue/worker, execute, restore) \u2014 no DB', bin: 'node', args: ['src/usat_apps/run_tests.js', 'modules/salesforce_merge'], cli: 'node src/usat_apps/run_tests.js modules/salesforce_merge' },
    { id: 11, label: 'E2E \u2014 UI/UX (Playwright)', desc: 'Browser suite: platform shell + participation map. Isolated build/port/creds \u2014 never touches the real dist. One-time: npx playwright install chromium', bin: 'npm', args: ['run', 'usat_apps_e2e'], cli: 'npm run usat_apps_e2e' },
    { id: 12, label: 'E2E \u2014 interactive runner', desc: 'Same suite in Playwright --ui (watch, step through, time-travel).', bin: 'npm', args: ['run', 'usat_apps_e2e_ui'], cli: 'npm run usat_apps_e2e_ui' },
  ]},
  { label: 'USERS & ACCESS', color: CYAN, items: [
    { id: 13, label: 'Add / update a user', desc: 'Create a web-app login (username/email, password, role)', bin: 'node', args: ['src/usat_apps/admin.js', 'add'], cli: 'node src/usat_apps/admin.js add' },
    { id: 14, label: 'List users', desc: 'Show .env recovery + stored web-app logins', bin: 'node', args: ['src/usat_apps/admin.js', 'list'], cli: 'node src/usat_apps/admin.js list' },
    { id: 15, label: 'Reset a user password', desc: 'Set a new password for an existing stored login', bin: 'node', args: ['src/usat_apps/admin.js', 'passwd'], cli: 'node src/usat_apps/admin.js passwd' },
    { id: 16, label: 'Remove a user', desc: 'Delete a stored login (prompts + confirm)', bin: 'node', args: ['src/usat_apps/admin.js', 'remove'], cli: 'node src/usat_apps/admin.js remove' },
    { id: 17, label: 'Show panel access', desc: 'Print the default + per-user panel allow-list + catalog', bin: 'node', args: ['src/usat_apps/admin.js', 'access'], cli: 'node src/usat_apps/admin.js access' },
  ]},
  { label: 'OPEN', color: GREEN, items: [
    { id: 18, label: 'Open built UI', desc: 'Production-style single-port app at :8022', open: `http://localhost:${PORT}`, cli: `open http://localhost:${PORT}` },
    { id: 19, label: 'Open dev UI', desc: 'Vite dev server (hot reload) at :5175', open: 'http://localhost:5175', cli: 'open http://localhost:5175' },
    { id: 20, label: 'Open via proxy (/)', desc: 'The app through the proxy at :8000/', open: 'http://localhost:8000/', cli: 'open http://localhost:8000/' },
    { id: 21, label: 'Check API status', desc: 'GET /api/status — backend health (public)', endpoint: '/api/status', cli: `curl http://localhost:${PORT}/api/status` },
    { id: 22, label: 'Check login / whoami', desc: 'GET /api/me — current user + role + panels (needs a signed-in session)', endpoint: '/api/me', cli: `curl http://localhost:${PORT}/api/me` },
    { id: 23, label: 'Show your modules', desc: 'GET /api/modules — the module catalog the nav is built from (needs a session)', endpoint: '/api/modules', cli: `curl http://localhost:${PORT}/api/modules` },
  ]},
  { label: 'PM2 (production)', color: RED, items: [
    { id: 24, label: 'pm2 start', desc: 'Run the server under pm2 (production)', bin: 'npm', args: ['run', 'pm2_start_usat_apps'], cli: 'npm run pm2_start_usat_apps' },
    { id: 25, label: 'pm2 restart', desc: 'Restart the pm2 process', bin: 'npm', args: ['run', 'restart_usat_apps'], cli: 'npm run restart_usat_apps' },
    { id: 26, label: 'pm2 stop', desc: 'Stop the pm2 process', bin: 'npm', args: ['run', 'stop_usat_apps'], cli: 'npm run stop_usat_apps' },
    { id: 27, label: 'pm2 logs', desc: 'Tail the pm2 logs', bin: 'npm', args: ['run', 'pm2_logs_usat_apps'], cli: 'npm run pm2_logs_usat_apps' },
  ]},
  { label: 'MODULES', color: CYAN, items: [
    { id: 28, label: 'Participation maps — data pipeline & ops →', desc: 'Rebuild region/zip/census/summary data, BigQuery load, build scope (opens the module menu)', bin: 'node', args: ['src/usat_apps/modules/participation_maps/menu.js'], interactive: true, cli: 'node src/usat_apps/modules/participation_maps/menu.js' },
    { id: 29, label: 'Salesforce merge — worker & ops →', desc: 'Start/stop the :8021 write worker, run smoke / worker-down tests, DB migrations, status (opens the module menu)', bin: 'node', args: ['src/usat_apps/modules/salesforce_merge/menu.js'], interactive: true, cli: 'node src/usat_apps/modules/salesforce_merge/menu.js' },
  ]},
];
