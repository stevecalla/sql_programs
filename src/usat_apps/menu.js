...
const SECTIONS = [
  ...
  { label: 'USERS & ACCESS', color: CYAN, items: [
    { id: 9, label: 'Add / update a user', desc: 'Create a web-app login (username/email, password, role)', bin: 'node', args: ['src/usat_apps/admin.js', 'add'], cli: 'node src/usat_apps/admin.js add' },
    { id: 10, label: 'List users', desc: 'Show .env recovery + stored web-app logins', bin: 'node', args: ['src/usat_apps/admin.js', 'list'], cli: 'node src/usat_apps/admin.js list' },
    { id: 11, label: 'Reset a user password', desc: 'Set a new password for an existing stored login', bin: 'node', args: ['src/usat_apps/admin.js', 'passwd'], cli: 'node src/usat_apps/admin.js passwd' },
    { id: 12, label: 'Remove a user', desc: 'Delete a stored login (prompts + confirm)', bin: 'node', args: ['src/usat_apps/admin.js', 'remove'], cli: 'node src/usat_apps/admin.js remove' },
    { id: 13, label: 'Show panel access', desc: 'Print the default + per-user panel allow-list + catalog', bin: 'node', args: ['src/usat_apps/admin.js', 'access'], cli: 'node src/usat_apps/admin.js access' },
  ]},
  ...
  { label: 'PM2 (production)', color: RED, items: [
    { id: 20, label: 'pm2 start', desc: 'Run the server under pm2 (production)', bin: 'npm', args: ['run', 'pm2_start_usat_apps'], cli: 'npm run pm2_start_usat_apps' },
    { id: 21, label: 'pm2 restart', desc: 'Restart the pm2 process', bin: 'npm', args: ['run', 'restart_usat_apps'], cli: 'npm run restart_usat_apps' },
    { id: 22, label: 'pm2 stop', desc: 'Stop the pm2 process', bin: 'npm', args: ['run', 'stop_usat_apps'], cli: 'npm run stop_usat_apps' },
    { id: 23, label: 'pm2 logs', desc: 'Tail the pm2 logs', bin: 'npm', args: ['run', 'pm2_logs_usat_apps'], cli: 'npm run pm2_logs_usat_apps' },
  ]},
];
