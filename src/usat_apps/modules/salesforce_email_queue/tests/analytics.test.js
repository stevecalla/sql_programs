...
// (build_report) — both driven by a FAKE pool, so no DB is needed. Ported 1:1 from the standalone POC's
// tests/analytics.test.js; only the require paths change for the platform layout (tests/ and metrics/ are
// siblings under the module, so the shared-analytics + DDL paths match metrics/events.js exactly).
