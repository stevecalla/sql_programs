'use strict';
// log_ring.js — in-memory ring buffer of the server's console output + a pm2 stats reader. Ported 1:1
// from the standalone app's admin/log_ring.js for the Email Queue admin → Logs panel. install(console)
// patches console.log/info/warn/error so every line is ALSO captured into a ~500-line ring (the real
// console still prints). The Logs panel reads the ring (GET + SSE tail). read_pm2() shells `pm2 jlist`.
...
function read_pm2(process_name) {
  return new Promise(function (resolve) {
    ...
    try { proc = spawn('pm2', ['jlist'], { shell: need_shell }); }
    catch (e) { return resolve({ under_pm2: false, reason: 'pm2 not available' }); }
    ...
    proc.on('close', function () {
      ...
      resolve({
        under_pm2: true, name: proc_rec.name, status: pm2_env.status || 'unknown',
        restarts: pm2_env.restart_time || 0, uptime_ms: uptime_ms,
        cpu: monit.cpu != null ? monit.cpu : null, memory_bytes: monit.memory != null ? monit.memory : null,
        pid: proc_rec.pid || null, matched: proc_rec.name === process_name
      });
    });
  });
}

module.exports = { install: install, push: push, tail: tail, subscribe: subscribe, read_pm2: read_pm2, MAX: MAX };
