'use strict';
// console_runner.js — runs the curated console_registry commands for the Email Queue admin → Operations
// panel. Ported 1:1 from the standalone app's admin/console_runner.js — same security model + lifecycle.
// Reached only behind require_admin. Allowlist by id: the client sends only { id, params, confirm }, never
// a command string. argv is assembled HERE from the registry item and spawned with shell:false for `node`.
// Destructive items require a typed confirm token = the command id. Per-run output cap + 15-min timeout +
// SSE subscribers + a sanitized audit line.
const path = require('path');
const { spawn } = require('child_process');
const registry = require('./console_registry');

// Repo root (sql_programs) — the re-pointed argv (src/usat_apps/run_tests.js, module check scripts, the
// metrics CLI) are relative to it, matching the module's menu.js.
...
function assemble_argv(item, params) {
  ...
  for (let i = 0; i < specs.length; i++) {
    ...
    if (p.type === 'enum') {
      ...
      if (!opt) { if (p.required) return { ok: false, error: 'invalid choice for ' + p.name }; continue; }
      (opt.args || []).forEach(function (a) { extra.push(a); });
      continue;
    }
    if (val === '') { if (p.required) return { ok: false, error: 'missing required field: ' + p.name }; continue; }
    if (p.type === 'int') { if (!/^\d+$/.test(val)) return { ok: false, error: p.name + ' must be a whole number' }; if (p.flag) extra.push(p.flag); extra.push(val); continue; }
    if (p.type === 'date') { if (!/^\d{4}-\d{2}-\d{2}$/.test(val)) return { ok: false, error: p.name + ' must be YYYY-MM-DD' }; if (p.flag) extra.push(p.flag); extra.push(val); continue; }
    if (p.type === 'path') { if (!valid_path(val)) return { ok: false, error: 'invalid folder for ' + p.name + ' (must be a relative path, no "..")' }; if (p.flag) extra.push(p.flag); extra.push(val); continue; }
...
function active_count() { let n = 0; runs.forEach(function (r) { if (r.status === 'running') n++; }); return n; }
...
function finish(run, status, code) {
  ...
  run.status = status; run.code = code; run.ended = Date.now();
  ...
  console.log('[eq-admin-console] run ' + run.run_id + ' (' + run.item.action + ') ' + status + ' code=' + code);
...
function start_run(item, params, confirm) {
  ...
  let child;
  try { child = spawn(bin, built.argv, { cwd: RUN_DIR, shell: need_shell, env: process.env }); }
  catch (e) { return { ok: false, error: 'failed to start: ' + e.message }; }
  const run_id = 'r' + (++seq) + '_' + Date.now().toString(36);
  const run = { run_id: run_id, item: item, bin: bin, argv: built.argv, status: 'running', code: null, lines: [], subs: new Set(), started: Date.now(), ended: null, capped: false, child: child };
  runs.set(run_id, run);
...
module.exports = {
  RUN_DIR: RUN_DIR, assemble_argv: assemble_argv, valid_path: valid_path,
  start_run: start_run, subscribe: subscribe, kill_run: kill_run, list_runs: list_runs,
