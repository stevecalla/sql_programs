'use strict';
// App-specific analytics aggregation for the Salesforce Email Queue module. Returns the shared "report
// contract" (structured `data` + human `sections`) consumed by the React /metrics/sf-email-queue
// dashboard. Ported from the standalone POC's metrics/metrics_report.js (same queries → same numbers),
// re-homed onto the platform: `pool` is injected (from store/db), and an `include_test` toggle mirrors
// the Merge module. Uses the shared utilities/analytics retention + render helpers.
...
async function build_report(pool, opts) {
  ...
  const ai = (await q(pool,
    'SELECT COUNT(*) calls, SUM(ai_ok=1) ok, SUM(ai_ok=0) failed, ' +
  ...
  // ---- health (whole table, unfiltered) ----
  const health = (await q(pool, "SELECT COUNT(*) rows_total, SUM(CASE WHEN is_test=1 THEN 1 ELSE 0 END) test_rows, DATE_FORMAT(MAX(created_at_mtn), '%b %e, %Y %l:%i %p') latest FROM `" + T + "`"))[0] || {};
  ...
  // ---- AI spend: REAL vs TEST vs TOTAL across the window (NOT is_test-filtered), + per env ----
  ...
  const data = {
    days: days, include_test: !!opts.include_test,
    ...
    funnel: [
      ...
      { stage: 'Acknowledgements', n: n0(sfw.acks) }
    ],
    health: {
      rows: n0(health.rows_total),
      test_rows: n0(health.test_rows),
  ...
  const report = {
    title: 'Salesforce Email Queue — Metrics (last ' + days + ' days)',
