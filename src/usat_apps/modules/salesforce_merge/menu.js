...
const SECTIONS = [
  ...
  { label: 'SALESFORCE AUTH · connection', color: GREEN, items: [
    { id: 16, label: 'SF auth — Sandbox · Auto', desc: 'Normal path — tries OAuth (External Client App), falls back to SOAP; reports which one connected.', bin: 'node', args: [`${M}/check_sf_auth.js`], cli: `node ${M}/check_sf_auth.js` },
    { id: 17, label: 'SF auth — Production · Auto', desc: 'Normal path — tries OAuth (External Client App), falls back to SOAP; reports which one connected.', bin: 'node', args: [`${M}/check_sf_auth.js`, '--prod'], cli: `node ${M}/check_sf_auth.js --prod` },
    { id: 18, label: 'SF auth — Sandbox · OAuth', desc: 'OAuth only (no fallback) — confirms the new auth is working.', bin: 'node', args: [`${M}/check_sf_auth.js`, '--oauth'], cli: `node ${M}/check_sf_auth.js --oauth` },
    { id: 19, label: 'SF auth — Production · OAuth', desc: 'OAuth only (no fallback) — confirms the new auth is working.', bin: 'node', args: [`${M}/check_sf_auth.js`, '--prod', '--oauth'], cli: `node ${M}/check_sf_auth.js --prod --oauth` },
    { id: 20, label: 'SF auth — Sandbox · SOAP', desc: 'SOAP only (the method retiring Summer 2027) — confirms it still works / flags when it goes offline.', bin: 'node', args: [`${M}/check_sf_auth.js`, '--soap'], cli: `node ${M}/check_sf_auth.js --soap` },
    { id: 21, label: 'SF auth — Production · SOAP', desc: 'SOAP only (the method retiring Summer 2027) — confirms it still works / flags when it goes offline.', bin: 'node', args: [`${M}/check_sf_auth.js`, '--prod', '--soap'], cli: `node ${M}/check_sf_auth.js --prod --soap` },
  ] },
  { label: 'DOSSIER · files & data', color: CYAN, items: [
    { id: 22, label: 'Check Files permission (sandbox)', desc: 'Does the SF write user identity + can it create Files (ContentVersion / ContentDocumentLink)?', bin: 'node', args: [`${M}/check_dossier_access.js`], cli: `node ${M}/check_dossier_access.js` },
    { id: 23, label: 'Check Files permission (PROD, live)', desc: 'Production check that actually creates + deletes a throwaway File to prove the write path', bin: 'node', args: [`${M}/check_dossier_access.js`, '--prod', '--live'], cli: `node ${M}/check_dossier_access.js --prod --live` },
    { id: 24, label: 'Extract a dossier → Downloads', desc: 'List recent dossiers, pick an id, write the .xlsx to <home>/Downloads (byte-exact)', bin: 'node', args: [`${M}/extract_dossier.js`], cli: `node ${M}/extract_dossier.js` },
    { id: 25, label: 'Pull a dossier from SQL — how', desc: 'Print the SELECT / node / app-endpoint ways to get the .xlsx out of MySQL', info: PULL_HELP },
    { id: 26, label: 'View a dossier in Salesforce (SOQL)', desc: 'Print the SOQL to find / download the attached File (by record or by name)', info: VIEW_SF_HELP },
    { id: 27, label: 'Repair loser file shares', desc: 'Re-link a restored loser’s Files that stayed on the survivor. Shows usage; run with --queue <id> (add --apply to write).', bin: 'node', args: [`${M}/repair_file_shares.js`], cli: `node ${M}/repair_file_shares.js --queue <id> [--apply]` },
  ] },
  { label: 'RESET · testing (destructive)', color: RED, items: [
    { id: 28, label: 'Reset merge tables — DRY RUN', desc: 'Show row counts for the 7 merge tool tables; changes nothing', bin: 'node', args: [`${M}/reset_merge_tables.js`], cli: `node ${M}/reset_merge_tables.js` },
    { id: 29, label: 'Reset merge tables — APPLY', desc: 'CLEARS the 7 merge tool tables (queue/snapshots/history/run/dossier); leaves the finder input', bin: 'node', args: [`${M}/reset_merge_tables.js`, '--apply'], cli: `node ${M}/reset_merge_tables.js --apply` },
  ] },
  { label: 'STRESS TEST · sandbox', color: CYAN, items: [
    { id: 30, label: 'Show cluster-size distribution', desc: 'Histogram of duplicate cluster sizes for the loaded dataset (default sandbox; add --env production). Shows the dataset stamp + lets you pick a representative min/max size band', bin: 'node', args: [`${M}/stress_test.js`, 'distribution'], cli: `node ${M}/stress_test.js distribution` },
    { id: 31, label: 'Clear run tables (stress)', desc: 'Empty the merge queue/history/snapshot/run/dossier tables for a clean run (finder input untouched). Type CLEAR to confirm', bin: 'node', args: [`${M}/stress_test.js`, 'clear'], cli: `node ${M}/stress_test.js clear` },
    { id: 32, label: 'Run merges (select → queue → approve → process)', desc: 'Interactive: env, source, count, batch, size band, seed; simulate by default (type MERGE + --execute for real writes). Report to /data', bin: 'node', args: [`${M}/stress_test.js`, 'run'], cli: `node ${M}/stress_test.js run` },
    { id: 33, label: 'Run merges — PARALLEL (worker cluster)', desc: 'Same as Run merges but enqueues ALL batches at once so 2 workers (start via item 2) drain them side by side — to test the cluster. Shows workers-active + finishes ~2x faster.', bin: 'node', args: [`${M}/stress_test.js`, 'parallel'], cli: `node ${M}/stress_test.js parallel` },
    { id: 34, label: 'Run merges — JOB FAN-OUT (Phase 1 test)', desc: 'Drives the REAL /merge/process fan-out: splits the approved sets with the same chunker + settings the endpoint uses into N chunk-runs sharing a job_id, then polls job_progress (batches/sets/workers). Needs the cluster (item 2). Set chunk < count to see the fan-out.', bin: 'node', args: [`${M}/stress_test.js`, 'job'], cli: `node ${M}/stress_test.js job` },
    { id: 35, label: 'Restore last merged sets (stress)', desc: 'Undo the currently-restorable (recently-merged) sets from the CLI + write a restore report. Type RESTORE to confirm', bin: 'node', args: [`${M}/stress_test.js`, 'restore'], cli: `node ${M}/stress_test.js restore` },
    { id: 36, label: 'Full sequence (clear → merge → restore)', desc: 'Runs the whole sequence into one workbook — prompts the options once', bin: 'node', args: [`${M}/stress_test.js`, 'sequence'], cli: `node ${M}/stress_test.js sequence` },
    { id: 37, label: 'Open stress report folder', desc: 'Open the /data folder where the stress-test Excel reports are written', bin: 'node', args: [`${M}/stress_test.js`, 'open'], cli: `node ${M}/stress_test.js open` },
  ] },
  { label: 'DIAGNOSTICS · data safety (read-only)', color: CYAN, items: [
    { id: 38, label: 'Probe donor/gift attachment (sandbox)', desc: 'Read-only #6 check: is a "Donor Gift Summary" a master-detail child (cascade-deleted by a merge → data-loss risk) or a lookup (SF re-parents, low risk)? Also flags if the integrated user can\'t see the object. Finds a foundation/donor duplicate to test.', bin: 'node', args: [`${M}/probe_donor_gift.js`], cli: `node ${M}/probe_donor_gift.js` },
    { id: 39, label: 'Probe donor/gift attachment (PRODUCTION)', desc: 'Same read-only probe against production (DESCRIBE + SELECT only, never writes).', bin: 'node', args: [`${M}/probe_donor_gift.js`, '--prod'], cli: `node ${M}/probe_donor_gift.js --prod` },
  ] },
];
