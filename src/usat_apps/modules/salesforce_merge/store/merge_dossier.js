...
async function build(opts = {}, deps = {}) {
  ...
  const REF_ROWS = [
    ...
    ['Lifecycle stamp (in Salesforce)', 'The usat_Was_Merged__pc marker written on the survivor Account by each action (flag on after MERGE / off after RESTORE|RECREATE; text “<ACTION> — <actor>”). Person-Account (__pc) projection of the Contact field; lives in Salesforce, not MySQL.', "SOQL:  SELECT Id, usat_Was_Merged__pc, usat_Was_Merged_Date__pc, usat_Was_Merged_By__pc FROM Account WHERE Id = '" + surv + "'"],
