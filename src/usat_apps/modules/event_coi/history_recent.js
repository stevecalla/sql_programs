...
async function main() {
  ...
  console.log('\n  ' + pad('#', 4) + pad('started', 17) + pad('event', 24) + pad('by', 10) +
    padL('req', 4) + padL('sub', 5) + padL('fail', 5) + padL('skip', 5) + '  status');
  console.log('  ' + '-'.repeat(78));
  if (!rows.length) console.log('  (no runs recorded yet)');
  rows.forEach((r, i) => {
    const flag = r.status === 'running' ? '  ←' : '';
    console.log('  ' + pad(i + 1, 4) + pad(fmtDate(r.started_at), 17) + pad(r.event_name, 24) + pad(r.ran_by, 10) +
  ...
  console.log('\n  counts by status');
  counts.forEach((c) => console.log('    ' + pad(c.status, 14) + padL(c.runs, 6) + (c.status === 'running' ? '  ←' : '')));
