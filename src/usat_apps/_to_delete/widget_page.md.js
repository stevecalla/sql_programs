...
// Client-side helpers (injected into the widget via .toString()): escape HTML, then linkify Markdown links
// [label](url) and bare http(s) URLs into anchors, so the assistant's answers can carry CLICKABLE links.
// Escaping runs first and we only emit anchors we build ourselves (http/https only) — no HTML injection.
...
function render(opts) {
  ...
  const cfg = { queue: String(o.queue || 'Team USA'), endpoint: '/api/public-chatbot/ask', theme: o.theme === 'dark' ? 'dark' : 'light', bubble: BUBBLES.indexOf(o.bubble) >= 0 ? o.bubble : 'plain' };
  const CLIENT = [
    "(function(){",
...
function render_loader() {
  return [
    ...
    "  f.src=origin+'/api/public-chatbot/widget?theme='+theme+(bubble?('&bubble='+encodeURIComponent(bubble)):'')+(color?('&color='+encodeURIComponent(color)):'');",
