...
**Mark-reviewed CLI (Step 12):** `cmd_mark_reviewed(sids, {created_by='cli:review'})` in `ask.js` (CLI: `--mark-reviewed <sid> [<sid> ...]`, menu option 16) is the terminal mirror of the dashboard's Reviewed? checkbox. For each sid, queries `event_analysis_roster` for the latest snapshot (`MAX(build_at)`) in the current year scope (`current_year_scope()` reads env vars), then dispatches by segment: matched pair (`Retained / Shifted / Tried to Return / Recovered`) → `cmd_add_match(sid_baseline, sid_analysis)` + `cmd_approve(sid_baseline)`; `Lost` → `cmd_add_segment('baseline', sid, 'Lost')` + `cmd_approve`; `New` → `cmd_add_segment('analysis', sid, 'New')` + `cmd_approve`. Tag `cli:review` distinguishes from `dashboard:review` in the audit trail. Idempotent (existing add returns `{status:'exists'}`, approve refreshes the signature). Returns `{inserted, exists, skipped, errors}`. Sids not in the latest snapshot are warned-and-skipped, not errored — one bad sid doesn't abort the rest. Requires the roster snapshot to exist (at least one build), so menu order is: build → mark-reviewed → rebuild to apply. 5 DB-backed tests in `tests/overrides.test.js` Step 12 use sentinel years (1999/2000) for fixture isolation. The inverse is `cmd_unmark_reviewed(sids)` (CLI: `--unmark-reviewed`, menu option 17): `UPDATE active=0, approved=0, approval_state=NULL WHERE created_by IN ('cli:review','dashboard:review') AND (sid_baseline=? OR sid_analysis=?) AND year-scope`. Only touches review-tagged rows so manually-created overrides on the same sid survive. Returns `{removed, missing, errors}`. Sids with no review marker bump `missing` (not an error). 3 DB tests cover the soft-delete + invariant, the missing-sid path, and the "leaves non-review rows intact" guarantee.
...
| Collapsible dashboard sections | Four `<details class="dash-section">` wrappers around major page areas: `#dash-section-insights` (Key findings card), `#dash-section-charts` (all chart `.row` blocks grouped as one), `#dash-section-table` (the roster table), `#dash-section-rebuild` (the rebuild card incl. ad-hoc years). HTML defaults: Charts and Table `open`; Insights and Rebuild closed (compact for the override-editor workflow). localStorage overrides per-section via keys `dash_collapse_insights / dash_collapse_charts / dash_collapse_table / dash_collapse_rebuild` (`'0'` = collapsed, `'1'` = open, missing = use HTML default). Boot wire-up in `dash_ov_init` iterates a `SECTION_KEYS` map and attaches a `toggle` listener that persists. CSS class `.dash-section-summary` styles the summary bar (light gray background, chevron that rotates on `[open]`). Operator-driven layout shrink for override-editor workflows. Covered by 4 tests in `tests/dashboard.test.js`: HTML structure of all 3 details + default-open behavior + localStorage='0' collapses at boot + toggle persists. |
