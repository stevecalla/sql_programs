...
const { query_create_region_table } = require('../queries/create_drop_db_table/query_create_region_table');
const { query_load_region_data } = require('../queries/load_data/query_load_region_data');
...
async function execute_load_region_data() {
    let pool;
    const startTime = performance.now();
    ...
    try {
        ...
        const step = `STEP #2.1:`;
        const step_info = `region_data`;

        const drop_query = await query_drop_table(table_name);
        console.log('drop query = ', drop_query);

        const create_query = await query_create_region_table(table_name);
        ...
        console.log(`STEP #3 - GET FILES IN DIRECTORY / LOAD INTO "usat region data" TABLE`);
        console.log(getCurrentDateTime());

        let rows_added = 0;

        const directory = await create_directory('usat_region_data');
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (currentFile.endsWith('.csv')) {
                ...
                const query_load = query_load_region_data(filePath, table_name);

                // Insert file into "region_data" table
