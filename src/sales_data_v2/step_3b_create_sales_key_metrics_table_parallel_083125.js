...
async function create_target_table(dstConn, update_mode, options, where_statement, FROM_STATEMENT) {
  ...
  if (update_mode === 'full') {
    ...
    console.log(`\nNote: Update mode is partial or updated_at thus did not drop the current table ${TARGET_TABLE_NAME}.`);
...
async function step_3b_create_sales_key_metrics_table_parallel(FROM_STATEMENT, pool, update_mode = 'updated_at', options) {
...
//   step_3b_create_sales_key_metrics_table_parallel(FROM_STATEMENT, pool, update_mode, options);
// }
