...
async function build_report(pool, opts) {
  ...
  // Headline window EXCLUDES is_test=1 (Sandbox / ?metrics_test=1) so test activity never inflates
  // the dashboard. Test rows are still counted in `health` and are purgeable.
  ...
  const queued = setmap.queue_add + setmap.queue_bulk_add + cmap.queue_add + cmap.queue_bulk_add; // sets if logged, else event count
