'use strict';
// User management CLI for the merge tool (mirrors the email-queue src/admin.js). Usage:
//   node admin.js add [user]        (prompts for password + role)
//   node admin.js list
//   node admin.js passwd <user>     (reset a password)
//   node admin.js remove <user>
//   node admin.js access            (show the panel-access config: default + per-user overrides)
// Stored users live OUTSIDE the repo (auth.json); .env recovery accounts (MERGE_ADMIN_*, MERGE_TEST_*)
// are managed in the repo-root .env, not here.
...
async function main() {
  ...
  if (cmd === 'access') {
    ...
    console.log('\npanels:', panel_access.PANEL_KEYS.map(function (k) { return k || '(dashboard)'; }).join(', '));
    return;
  }
  ...
  console.log('usage: node admin.js add [user] | passwd <user> | list | remove <user> | access');
}
main();
