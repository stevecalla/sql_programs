...
// fetch the parent already did; omit it and the funnel fetches /api/dashboard itself.
  ...
  return (
    <>
      <div className="funnel fade-in">
        <Step k="All accounts" v={fmt(d.total_accounts)} sub="every person record" sep="" to="/accounts" />
        <Step k="Duplicate accounts" v={fmt(d.accounts_in_clusters)} sub="records in a cluster" tone="accent" to="/duplicates" />
        <Step k="Duplicate pairs" v={fmt(d.duplicate_pairs)} sub="matches between two" to="/duplicates" />
        <Step k="Duplicate clusters" v={fmt(d.clusters)} sub="one unique duplicate" to="/duplicates" />
      </div>
...
export function MergeIdFunnel({ d: passed }) {
  ...
  return (
    <>
      <div className="funnel fade-in">
        <Step k="Accounts compared" v={fmt(reviewed)} sub="Platform merge IDs vs. Salesforce duplicates" sep="" tone="accent" to="/merge-id" />
        <Step k="In both" v={fmt(inBoth)} sub="has a merge ID & flagged as a duplicate" sep="→" tone="green" to="/merge-id" />
        <Step k="Only in merge IDs" v={fmt(sfOnly)} sub="has a merge ID, not flagged as a duplicate" sep="+" tone="amber" to="/merge-id" />
        <Step k="Only in duplicates" v={fmt(onlyOurs)} sub="flagged as a duplicate, no merge ID" sep="+" to="/merge-id" />
      </div>
      <p className="funnel-note">
