...
const SECTIONS = [
  { label: 'RUN', color: YELLOW, items: [
    ...
    { id: 4, label: 'Build the web app', desc: 'Compile React to web/dist (served at root :8020)', run: 'salesforce_merge_build', cli: 'npm run salesforce_merge_build' },
    { id: 5, label: 'Build for proxy (/merge base)', desc: 'Build with Vite base /merge/ for usat-app behind the proxy', run: 'salesforce_merge_build_proxy', cli: 'npm run salesforce_merge_build_proxy' },
    { id: 6, label: 'Start built server (:8020)', desc: 'Express serves the built UI + API on one port', run: 'salesforce_merge_server', cli: 'npm run salesforce_merge_server' },
    { id: 7, label: 'Start proxy (:8000)', desc: 'Reverse proxy; serves the app at :8000/merge/', run: 'proxy_server', cli: 'npm run proxy_server' },
  ]},
  ...
  { label: 'OPEN', color: GREEN, items: [
    ...
    { id: 11, label: 'Open built UI', desc: 'Production-style single-port app at :8020', open: 'http://localhost:8020', cli: 'open http://localhost:8020' },
    { id: 12, label: 'Open via proxy (/merge)', desc: 'The app through the proxy at :8000/merge/', open: 'http://localhost:8000/merge/', cli: 'open http://localhost:8000/merge/' },
    { id: 13, label: 'Check API status', desc: 'Ping the backend health endpoint', status: true, cli: 'curl http://localhost:8020/api/status' },
  ]},
  { label: 'PM2 (production)', color: RED, items: [
    { id: 14, label: 'pm2 start', desc: 'Run the server under pm2 (production)', run: 'pm2_start_salesforce_merge', cli: 'npm run pm2_start_salesforce_merge' },
    { id: 15, label: 'pm2 restart', desc: 'Restart the pm2 process', run: 'restart_salesforce_merge', cli: 'npm run restart_salesforce_merge' },
    { id: 16, label: 'pm2 stop', desc: 'Stop the pm2 process', run: 'stop_salesforce_merge', cli: 'npm run stop_salesforce_merge' },
    { id: 17, label: 'pm2 logs', desc: 'Tail the pm2 logs', run: 'pm2_logs_salesforce_merge', cli: 'npm run pm2_logs_salesforce_merge' },
  ]},
];
