...
async function step_1_sales_actual_v_goal_data(options) {
    ...
    // ============================================================================
    // PRIOR VERSION — retained (commented out) for reference.
    // Goal-anchored LEFT JOIN (FROM sales_goals LEFT JOIN sales_actuals) dropped
    // any category that existed in actuals but had no matching goal row, so totals
    // did not reconcile back to source (e.g. Bronze - Bike/Run/Swim, Bronze
    // Community Membership, Elite 2-Year membership). Replaced by the spine-based
    // full-outer assembly below.
    // ============================================================================
    /*
    return `
    ...
    */
    ...
    // ============================================================================
    // NEW VERSION — spine-based full-outer join.
    // Grain = the UNION of all keys (month/type/category) from actuals AND goals,
    // so every actual-only category flows through with real actual units + 0 goal,
    // and every goal-only category is preserved with 0 actuals. Totals reconcile
    // back to source. The old "Unknown" UNION ALL block and the "WHERE NOT
    // (Unknown AND zero-actual)" filter are no longer needed — the spine covers
    // Unknown naturally. Output columns are identical (names/order) to the prior
    // version, so step_2, the BigQuery load query, and the BQ schema are unaffected.
    // ============================================================================
    return `
        -- CREATE ACTUAL VS GOAL DATA
        DROP TABLE IF EXISTS ${table_name};
        ...
        CREATE TABLE ${table_name} AS
            ...
            sales_goals AS (
                SELECT
                    ...
                    -- category sort order using both type_goal and category_goal
                    CASE
                        -- adult_annual
                        WHEN real_membership_types_sa = 'adult_annual' AND new_member_category_6_sa = '1-Year $50' THEN 1
                        WHEN real_membership_types_sa = 'adult_annual' AND new_member_category_6_sa = '3-Year' THEN 2
                ...
                WHERE purchased_on_year_adjusted_mp = ${year}
                GROUP BY 1, 2, 3, 4, 5
                -- NOTE: prior "Unknown" UNION ALL block removed; the all_keys spine below
                --       now retains every actual-only category (incl. Unknown) automatically.
            ),
            all_keys AS (
                -- Full set of month/type/category keys present in EITHER actuals or goals.
                SELECT month_actual AS k_month, type_actual AS k_type, category_actual AS k_category
                FROM sales_actuals
                UNION
                SELECT month_goal, type_goal, category_goal
                FROM sales_goals
            )
            SELECT
                -- SALES GOAL DATA (keyed off the spine so it is always populated)
                k.k_month    AS month_goal,

                k.k_type     AS type_goal,
                k.k_category AS category_goal,

                IFNULL(sg.sales_rev_2025_goal, 0)    AS sales_rev_2025_goal,
                IFNULL(sg.sales_rev_2024_goal, 0)    AS sales_rev_2024_goal,
                IFNULL(sg.sales_units_2025_goal, 0)  AS sales_units_2025_goal,
                IFNULL(sg.sales_units_2024_goal, 0)  AS sales_units_2024_goal,
                IFNULL(sg.rev_per_unit_2025_goal, 0) AS rev_per_unit_2025_goal,
                IFNULL(sg.rev_per_unit_2024_goal, 0) AS rev_per_unit_2024_goal,

                -- SALES ACTUAL DATA
                IFNULL(sa.month_actual, k.k_month) AS month_actual,
                IFNULL(sa.quarter_actual,
                    CASE
                        WHEN k.k_month IN (1,2,3) THEN 1
                        WHEN k.k_month IN (4,5,6) THEN 2
                        WHEN k.k_month IN (7,8,9) THEN 3
                        ELSE 4
                    END) AS quarter_actual,
                IFNULL(sa.year_actual, ${year}) AS year_actual,

                CASE WHEN k.k_month =  MONTH(CURRENT_DATE) THEN 1 ELSE 0 END AS is_current_month,
                CASE WHEN k.k_month <= MONTH(CURRENT_DATE) THEN 1 ELSE 0 END AS is_year_to_date,

                -- SEGMENTS
                IFNULL(sa.type_actual, k.k_type)         AS type_actual,
                IFNULL(sa.category_actual, k.k_category) AS category_actual,
                COALESCE(sa.category_sort_order_actual, sg.category_sort_order_goal, 999) AS category_sort_order_actual,
                ...
                -- ABSOLUTE DIFFERENCE = GOAL VS 2025 ACTUALS
                IFNULL(sa.sales_rev_2025_actual, 0)    - IFNULL(sg.sales_rev_2025_goal, 0)    AS goal_v_actual_rev_diff_abs,
                IFNULL(sa.sales_units_2025_actual, 0)  - IFNULL(sg.sales_units_2025_goal, 0)  AS goal_v_actual_units_diff_abs,
                IFNULL(sa.rev_per_unit_2025_actual, 0) - IFNULL(sg.rev_per_unit_2025_goal, 0) AS goal_v_actual_rev_per_unit_diff_abs,

                -- ABSOLUTE DIFFERENCE = 2025 ACTUALS VS 2024 ACTUALS
                IFNULL(sa.sales_rev_2025_actual, 0)    - IFNULL(sa.sales_rev_2024_actual, 0)    AS "2025_v_2024_rev_diff_abs",
                IFNULL(sa.sales_units_2025_actual, 0)  - IFNULL(sa.sales_units_2024_actual, 0)  AS "2025_v_2024_units_diff_abs",
                IFNULL(sa.rev_per_unit_2025_actual, 0) - IFNULL(sa.rev_per_unit_2024_actual, 0) AS "2025_v_2024_rev_per_unit_diff_abs",
            ...
            FROM all_keys AS k
                LEFT JOIN sales_goals AS sg
                    ON  k.k_month    = sg.month_goal
                    AND k.k_type     = sg.type_goal
                    AND k.k_category = sg.category_goal
                LEFT JOIN sales_actuals AS sa
                    ON  k.k_month    = sa.month_actual
                    AND k.k_type     = sa.type_actual
                    AND k.k_category = sa.category_actual
