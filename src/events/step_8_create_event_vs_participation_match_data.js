...
const { query_create_event_vs_participation_match_table } = require('../queries/create_drop_db_table/query_create_event_vs_participation_match_table');

const { 
  step_8_query_event_vs_participation_match_data,
  query_create_mtn_utc_timestamps 
} = require('../queries/events/step_8_get_event_vs_participation_match_data');
...
async function execute_create_event_vs_participation_match() {
  const BATCH_SIZE   = 500;
  const TABLE_NAME   = 'event_vs_participation_match_data';
  const TABLE_STRUCTURE  = await query_create_event_vs_participation_match_table(TABLE_NAME);
  ...
  // Get mtn & utc time stamps
  const sql_timestamps = await query_create_mtn_utc_timestamps();
  const [multi_result] = await dst.query(sql_timestamps);
  const rowsArray = multi_result.find(Array.isArray) || [];
  const { created_at_mtn, created_at_utc } = rowsArray[0] || {};
  console.log('time stamps =', created_at_mtn, created_at_utc);
  ...
  try {
    ...
    // 3) Stream rows from src.events into an internal 
    const stream = src
      .query(await step_8_query_event_vs_participation_match_data(created_at_mtn, created_at_utc))
      .stream()
    ;
...
// execute_create_event_vs_participation_match().catch(err => {
//     console.error('Stream failed:', err);
//     process.exit(1);
// });

module.exports = {
  execute_create_event_vs_participation_match,
};
