 ...
 * step_2a_create_duplicate_message.js. Uses a temp dir (no /data dependency).
 *
...
describe('find_latest_by_base', () => {
    test('picks the most recently modified match for a base name', () => {
        const dir = make_fixture_dir();
        try {
            // add a newer exact file; it should win on mtime
            const newer = path.join(dir, 'account_duplicates_sf_import_2026-06-04_12-00-00.csv');
            fs.writeFileSync(newer, 'a,b\n1,x\n2,y\n3,z');
...
describe('execute_get_duplicate_report', () => {
    test('summarizes the latest files (dir override)', async () => {
        const dir = make_fixture_dir();
        try {
            const all = await execute_get_duplicate_report('all', dir);
            ...
            const exact = await execute_get_duplicate_report('exact', dir);
    ...
    test('missing directory -> has_output false, zero counts', async () => {
        const out = await execute_get_duplicate_report('all', path.join(os.tmpdir(), 'sfdup-does-not-exist-xyz'));
...
describe('create_duplicate_message', () => {
    test('includes the counts and a freshness note', () => {
        const { main_message_text } = create_duplicate_message(
            { exact: 2, fuzzy_pair: 1, fuzzy_group: 0 },
            { file_selector: 'all', age_minutes: 3, mode: 'latest', regenerated: false }
        );
