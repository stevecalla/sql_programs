...
<body>
  ...
  <h2>Dashboard</h2>
  <p>The interactive dashboard generated by <code>node src/event_analysis/build_all.js</code>:</p>
...
curl "http://localhost:${DEFAULT_PORT}/api/events?year=${analysis_year}"</pre>
  ...
  return await new Promise((resolve, reject) => {
    const server = app.listen(port, () => {
      ...
      if (!silent) {
        ...
        console.log(`  → http://localhost:${actual}/output/dashboard.html (dashboard)`);
        console.log(`  → http://localhost:${actual}/api/status\n`);
